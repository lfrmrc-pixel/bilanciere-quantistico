...
\section{Evolutionary Interpretation of the Four Variables}\label{interpretazione-evolutiva-quattro-variables}

\Hypoth{} \textbf{The four variables as evolutionary signature.} Each
variable of \(\mathbb{V}^{(4)}\) has a phenomenological definition
(Ch.~5) and an \emph{evolutionary causal microfoundation} (§D.4, CPGG
framework). The two readings do not replace each other: they are
layered, offering the analyst diagnostic tools at different levels of
abstraction.
...
Variable
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
DEQ$^2$ definition (Ch.~3--5)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Evolutionary microfoundation (\Hypoth{})
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\(\Phi\) & Kuramoto order parameter: phase alignment of the \(\Pi\)
field & Systemic signature of the \(rB > C\) regime (Hamilton 1964):
\(\Phi > 0\) when cooperative mechanisms prevail over
extraction \\ \addlinespace
\(A\) & Convexity of \(\Pi\) with respect to volatility \(\sigma\) & Robustness
of the \(rB > C\) condition under perturbations: \(A > 0\) indicates that
the system gains cooperation from shocks rather than losing it \\ \addlinespace
\(\Omega\) & Ratio of dissipated to useful energy (Neijuan) & CPGG-derived
coefficient: \(\Omega \approx \phi\eta r/(r-1)\); measures the amount of
extraction that the sub-network \(\mathcal{E}\) operates on the cooperative
field \\ \addlinespace
\(\varepsilon_{\text{dis}}\) & Moral disengagement coefficient
(Bandura/Milgram) & Endogenous degradation of the institutions maintaining
\(rB > C\): increases with \(\phi > \phi^*\) through the sigmoid
\(\mathcal{S}(\mu[\phi - \phi^*])\) \\
\end{longtable}}
}

\Proved{} \textbf{Integrated reading.} A system with high \(\Phi\),
positive \(A\), low \(\Omega\), and contained \(\varepsilon_{\text{dis}}\)
is a system in which: (i) the condition \(rB > C\) holds for the majority
of sub-actors; (ii) the system is robust to cooperative shocks; (iii) the
extractive fraction \(\phi\) is below the critical threshold \(\phi^*\);
(iv) normative institutions are not degraded. This is precisely the
position of the \textbf{Quantum Balance} in Ch.~9 --- and now we know
\emph{why} that position is stable, not merely \emph{that} it is stable.

\Hypoth{} \textbf{The system as Ostrom commons at geopolitical scale
(\Hypoth{}).} The normative problem of Ch.~12 translates, in the
evolutionary reading, into designing the institutional conditions ---
Ostrom's (1990) eight principles scaled to geopolitics --- that maintain
\(\phi < \phi^*\) and hence \(\Phi > 0\) in the long run. The DEQ$^2$
provides the diagnostics; the evolutionary apparatus provides the
\emph{why} that diagnostics is necessary.
