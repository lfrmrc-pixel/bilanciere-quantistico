\chapter{Risky Dated Predictions: Falsificabilità
Pubblica}\label{predizioni-rischiose-datate-falsificabilituxe0-pubblica}

\begin{quote}
\textbf{Core argument:} una teoria sociale che non si espone alla
falsification \emph{datata} è indistinguibile da una narrazione. Questo
chapter aggrega le sedici risky predictions della DEQ$^2$ --- tredici
followste dai Ch.~7-9 sui tre attori sistemici (cinque USA, quattro
China, quattro Brazil), tre followste dal Ch.~10 sull'aggregato globale
...
ciascuna predizione è moreover specificato \emph{cosa accadrebbe alla
teoria se fallisse}: la falsification di alcune predizioni è gestibile
come local correction, quella di altre forzerebbe la riformulazione
structural dell'apparato.
\end{quote}



\section{DEQ Criteria$^2$ per una Predizione
...
\item
  ...
  verificationbile da terzi indipendenti;
\item
  \textbf{Probabilità soggettiva esplicita}:
  \(\Pr(\mathcal{P} \text{ vera}) = p\) è dichiarata, \(p \in (0, 1)\);
\item
  \textbf{Confollowsnza teorica}: è dichiarato cosa accade alla DEQ$^2$ se
  \(\mathcal{P}\) fallisce --- \emph{local correction},
  \emph{parametersc adjustment}, oppure \emph{riformulazione
  structural}.
\end{enumerate}
...
aggiornamento esplicito delle probabilità posteriori sui parameters della
...
\subsection{USA --- Decoherent Hegemon (Cap.
...
\(0{,}70\) & local (parameter \(\Phi\)) \\
\textbf{U2} & Quota dollaro nelle riserve IMF COFER \(< 55\%\) & Q4 2027
& \(0{,}55\) & IMF COFER trimestrale & \(0{,}55\) & local (parameter
\(\sigma\) esogena) \\
\textbf{U3} & Quota dollaro \(< 52\%\) & Q4 2030 & \(0{,}52\) & IMF
COFER trimestrale & \(0{,}50\) & local (cumulativa con U2) \\
\textbf{U4} & Vetoes presidenziali \(> 15\) + emergesncy docket SCOTUS
\(> 30\) in 12 mesi & gennaio-luglio 2027, primo anno Congresso 119$^\circ$ &
\(15\) vetoes, \(30\) emergesncy & Federal Register + SCOTUSblog &
\(0{,}55\) & parametersca (\(\Omega\) acuta vs cronica) \\
\textbf{U5} & Correlazione Musk visibility--polarizzazione USA
\(r > 0{,}3\) & calibrazione 2022-2026 & \(r = 0{,}3\) & dataset
compilato (Ch.~7) & \(0{,}65\) & local (modello
\(\zeta_{\text{Sol}}\)) \\
\end{longtable}}
}

\subsection{China --- Rigid Synchronized (Cap.
...
\bottomrule\noalign{}
\endlastfoot
\textbf{C1} & Deflazione Chinese \(> 12\) trimestri consecutivi & Q4 2026
& \(12\) trim. & NBS / PIIE / WB & \(0{,}65\) & local (\(\Omega\)
misurato) \\
\textbf{C2} & \(P(\text{evento *bloqueo* Taiwan})\) in 2027-2029,
conditional a (deflazione \(\geq 12\) trim.) \(\wedge\) (GDP \(< 4\%\))
\(\wedge\) (nascite \(< 5/1000\)) & 2027-2029 &
\(P \in [0{,}25, 0{,}40]\) & corso eventi + INSS / AEI / Crisis Group &
\(0{,}50\) entro intervallo & structural se \(P\) fuori intervallo \\
\textbf{C3} & Yuan in IMF COFER \(< 4\%\) nel 2027 e \(< 5\%\) nel 2030
& Q4 2027, Q4 2030 & \(4\%\), \(5\%\) & IMF COFER trimestrale &
\(0{,}60\) & local (modello dedollarizzazione) \\
\textbf{C4} & PPI Chinese in territorio negativo \(\geq 5\) anni cumulati
& Q4 2027 & \(5\) anni & NBS PPI mensile & \(0{,}70\) & local
(anti-involution efficacia) \\
\end{longtable}}
}

\subsection{Brazil --- Quantum Balance (Cap.
...
meta-stima soggettiva & parametersc adjustment al cambiare
evidenze \\
\textbf{B2} & Se A/B vince: TFFF impegni \(> 5\)B USD entro fine 2027 e
BRICS BMG operational entro Q2 2027 & Q4 2027, Q2 2027 & \(5\)B,
\emph{operational} & comunicati BRICS, World Bank & \(0{,}55\)
conditional & local (operatività iniziativa) \\
\textbf{B3} & Se A/B vince: V-Dem Brazil Liberal Democracy Index
\(> 0{,}65\) & report 2028 (anno 2027) & \(0{,}65\) &
\url{https://v-dem.net} & \(0{,}65\) conditional & local (\(\Phi\)
Brazil) \\
...
ufficiali & \(0{,}60\) conditional & structural (la
\emph{solitonizzazione} discorsiva è il meccanismo principale del Cap.
7.3) \\
\end{longtable}}
}

\Proved{} \textbf{Sintesi delle 13 predizioni Ch.~7-9.} Probabilità
soggettiva mediana: \(p \approx 0{,}60\). Distribuzione: 2 strutturali
(C2, B4), 2 di meta-stima (B1, U5), 9 locali. Il fallimento di una
predizione local aggiusterebbe parameters specifici; il fallimento di
una predizione structural costringerebbe a ripensare l'apparato
corrispondente.
...
--- operazionalizzato come \emph{almeno due fra USA, China, UE in
\(\mathcal{R}_-\)} & dicembre 2031 & \(\geq 2\) attori in
\(\mathcal{R}_-\) & aggregazione V-Dem, Pew, Brand Finance, IMF, NBS al
2031 & \(0{,}80\) & structural (forma di \(T_s^{(2),\text{sys}}\)) \\
\textbf{G2} &
\(P(\text{sistema globale 2031 con almeno un $\Phi$-engine globale operativo})\)
--- operazionalizzato come Brazil \(C_{\text{DEQ}}^2 \geq 1\)
\emph{oppure} nuovo attore equivalente & dicembre 2031 &
\(C_{\text{DEQ}}^2 \geq 1\) per almeno un attore & calcolo dashboard
SPC-DEQ & \([0{,}55, 0{,}70]\), conditional a B1 & local (dipende da
B1) \\
\textbf{G3} & China: divergenza \(d^{\text{oss}} - d^{\text{spont}}\)
crescente, con \emph{fatica del \(K\) imposto} visibile & finestra di
remark 2029-2031 &
\(\Delta(d^{\text{oss}} - d^{\text{spont}}) > 0{,}10\) in 24 mesi &
dashboard SPC-DEQ + V-Dem + NBS & \(0{,}55\) & structural (definition
di \emph{Rigid Synchronized} del Ch.~8.0) \\
\end{longtable}}
}

\Hypoth{} \textbf{Lettura.} G1 e G2 sono \emph{complementari}: la teoria
afferma simultaneamente che il sistema globale 2031 sarà in decoherence
\emph{e} che esiste con probabilità \(\approx 0{,}55\)-\(0{,}70\) un
\(\Phi\)-engine operational. La conferma di entrambe
(\(\Pr \approx 0{,}50\)) consoliderebbe il quadro M1 del Ch.~10.4. La
conferma di G1 con falsification di G2 (\(\Pr \approx 0{,}25\))
...
2026}\label{la-predizione-meta-lelezione-Brazilian-del-4-ottobre-2026}
...
Bayesian updating della probabilità del Mondo M1/M2 \\
...
Falsificazione}\label{protocollo-di-pubblicazione-e-falsification}

\Hypoth{} \textbf{Pre-registrazione.} Le sedici predizioni di questo
chapter sono \emph{pre-registrate} alla data di pubblicazione del libro
(target 2026 Q3 --- KDP IT/EN/ES/PT). La pre-registration consiste
...
\tightlist
\item
  \textbf{Table delle predizioni} (sezioni 11.1, 11.2) con tutti i
  campi (1)-(5);
\item
  ...
  \tightlist
  \item
    file CSV con tutte le predizioni e i loro stati (pending / vera /
    falsa);
  \item
    script Python di calcolo automatico delle predizioni quantitative
    dai dataset pubblici;
  \item
    log dei \emph{checkthennt} trimestrali con re-stima probabilità
    soggettive;
  \end{itemize}
\item
  ...
  \item
    Bayesian updating delle probabilità;
  \item
    eventuali correzioni alla teoria (locali/parametersche/strutturali).
  \end{itemize}
\end{enumerate}

\Proved{} \textbf{Confollowsnza editoriale.} Il libro non è
...
\emph{tutti} gli assi --- incluso \(z\) (dialectical complexity) through
l'esposizione esplicita alla falsification.
...
Predizione
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Fallimento $\to$ Confollowsnza per DEQ$^2$
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
del Ch.~6.6 va riformulato. Locale-parametersca. \\
C1 & Deflazione finisce first: anti-involution funziona. Aggiustamento
\(\Omega^{\text{Cina}}\). Locale. \\
C2 & Nessun bloqueo malgrado condizioni soddisfatte: hypothesis
\emph{Taiwan come valvola} va rivista. \textbf{Strutturale}. \\
C3 & Yuan oltre \(5\%\) entro 2030: la dedollarizzazione converge sullo
yuan, contro la tesi Ch.~8.6. Locale-parametersca. \\
C4 & PPI positivo first del 2027: anti-involution efficace.
...
B4 & (conditional a B1 falsa) fallimento --- Brazil resiste sotto
destra radicale: la \emph{solitonizzazione} del Ch.~7.3 va ripensata
...
\textbf{Strutturale}. \\
G2 & Brazil fallisce \(C_{\text{DEQ}}^2 \geq 1\) malgrado A/B: la
definition di \(\Phi\)-engine è insufficiente. \textbf{Strutturale}. \\
G3 & China mantiene \(d^{\text{oss}} \approx d^{\text{spont}}\): la
categoria \emph{Rigid Synchronized} non descrive la China come
...
U5). Il fallimento di una predizione structural impone
...
conditional debole): \(\Pr \approx (1-0{,}55)^5 \approx 0{,}018\) ---
...
invalidare l'apparato; il libro è therefore un \emph{pre-registered
prediction set} il cui valore epistemico cresce o diminuisce in
proporzione alla performance verificationbile della teoria.
...
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Tipo di falsification
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Ch.~7 USA & 5 & \(0{,}60\) & 4 locali, 1 parametersca/meta (U5) \\
Ch.~8 China & 4 & \(0{,}60\) & 3 locali, 1 structural (C2) \\
Ch.~9 Brazil & 4 & meta + condizionali & 2 condizionali, 1 structural
(B4), 1 meta (B1) \\
Ch.~10 globali & 3 & \(0{,}65\) & 2 strutturali (G1, G2 e G3 entrambe),
1 conditional \\
...
\Hypoth{} \textbf{Tesi conclusiva.} Le predizioni di questo chapter
sono il \emph{contenuto empirico futuro} della DEQ$^2$. La Part V del
libro (Ch.~12-13) costruirà instead il \emph{contenuto normativo}: dato
...
\bottomrule\noalign{}
\endlastfoot
1 & Cinque criteri DEQ$^2$ per risky prediction & \Hypoth{}
methodology \\
2 & Tredici predizioni Ch.~7-9 aggregate con campi (1)-(5) & \Proved{}
complete aggregation \\
3 & Tre predizioni di sistema globale dal Ch.~10 & \Hypoth{}
followszione coerente \\
4 & B1 come predizione meta che condiziona la cascata & \Hypoth{}
architettura logica \\
5 & Protocollo di pre-registration e repository GitHub & \Hypoth{}
...
\Conj{} stima sotto indipendenza conditional \\
8 & \(5\) strutturali + \(9\) locali/condizionali + \(2\) meta &
\Proved{} complete classification \\
...
\section{Chiusura della Part IV e Opening of Part
V}\label{chiusura-della-parte-iv-e-apertura-della-parte-v}

La \textbf{Part IV} è ora completa. Ch.~10 ha tradotto l'apparato in
...
Ch.~11 ha aggregato le 16 risky predictions datate con criteri
popperiani operativi e protocollo di falsification pubblica.

La \textbf{Part V} può ora costruire il contenuto normativo:

\begin{itemize}
\tightlist
\item
  \textbf{Ch.~12} --- \emph{Cosa fare}: strategie individuali (livello
