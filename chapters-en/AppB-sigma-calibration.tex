\chapter{\texorpdfstring{Empirical Calibration of
\(\Sigma^{\text{geo}}\) a Partire da
\(\Sigma^{\text{comm}}\)}{Empirical Calibration of \textbackslash Sigma\^{}\{\textbackslash text\{geo\}\} a Partire da \textbackslash Sigma\^{}\{\textbackslash text\{comm\}\}}}\label{calibrazione-empirica-di-sigmatextgeo-a-partire-da-sigmatextcomm}

\begin{quote}
\textbf{Core argument:} la matrice di covarianza degli stati
egemonici \((s_i, s_{ii}, s_{iii})\) può essere stimata, in first
...
Nel Ch.~4, \(T_s^{(2),\text{sys}}\) utilizza implicitamente una
struttura di covarianza tra gli assi \((s_i, s_{ii}, s_{iii})\) del
sistema egemonico --- necessaria, for example, per definire \(\sigma\)
...
pubblicazione precedente. La presente appendix propone una procedura di
calibrazione fondata sulla relazione formal tra scala micro
(Topografia) e scala macro (Geometria) stabilita nei Ch.~3-4.



\section{Setup Formale: la Mappa di
Aggregazione}\label{setup-formal-la-mappa-di-aggregazione}

\subsection{Definizione}\label{definition}

Sia \(\mathbf{c}_j = (x_j, y_j, z_j)^T\) il vettore comunicativo
dell'coupling elementare \(j\) all'interno di un sistema sociale, e
\(\mathbf{s} = (s_i, s_{ii}, s_{iii})^T\) il vettore di stato del
sistema. Postuliamo l'esistenza di una \textbf{mappa di aggregazione
lineare} \(J \in \mathbb{R}^{3 \times 3}\) such that:
...
first approssimazione --- non lineare in regimi estremi (collapse,
phase transition) ma adeguata in regimi quasi-stazionari, che è il
...
identificando \(J\), we obtain \(\Sigma^{\text{geo}}\) senza necessità
...
Identitaria}\label{strategia-a-hypothesis-nulla-identitaria}
...
non-ortogonalità si conserva sotto aggregazione. È l'hypothesis più
...
\tightlist
\item
  \(\alpha_1\): peso della formal coherence micro \(x\) nella coesione
  interna macro \(s_i\);
\item
  \(\alpha_2\): peso della technical precision micro \(y\) nella capacità
  materiale macro \(s_{ii}\);
\item
  \(\alpha_3\): peso della dialectical complexity micro \(z\) nella
  legitimacy macro \(s_{iii}\).
\end{itemize}
...
Ipotesi più generale: \(J\) è \(3 \times 3\) piena, \(9\) parameters
liberi. Permette di catturare effetti come ``il discorso tecnico genera
material capacity ma anche coesione'' (\(J_{12} > 0\)) o ``la
dialectical complexity produce legitimacy ma erode coesione''
(\(J_{31} > 0,\ J_{11}\) ridotto).

\Conj{} \textbf{Limite.} \(9\) parameters richiedono almeno 9 punti dati
...
where la base dati può essere costruita.
...
\bottomrule\noalign{}
\endlastfoot
\(s_i\) --- internal cohesion & World Bank Government Effectiveness &
Polity IV, V-Dem Liberal Democracy & WB / Polity / V-Dem \\
\(s_{ii}\) --- material capacity & PIL nominale $\times$ Economic Complexity
Index & Spesa militare, brevetti, energia & WB / IMF / OEC \\
\(s_{iii}\) --- ethical-discursive legitimacy & Brand Finance Soft Power
...
standardizzate per formal coherence, technical precision, complessità
dialettica.
...
materiale) è il più alto: la technical precision del discorso si traduce
\emph{più che proporzionalmente} in material capacity macroscopica.
...
\section{Confollowsnze per la DEQ$^2$}\label{confollowsnze-per-la-dequxb2}
...
Questo è il livello base di volatilità aggregata from which le predizioni
del Ch.~11 partono.

\subsection{\texorpdfstring{Calibrazione di
\(\Phi\)}{Calibrazione di \textbackslash Phi}}\label{calibrazione-di-phi}

\Hypoth{} La densità di coupling \(K\) del Ch.~4 può essere
...
(cooperazione interna effettiva), e therefore sono più probabilmente sopra
...
dovrebbe avere \(R^2 \geq 0{,}75\) with respect tolla \(\Sigma^{\text{geo}}\)
...
\Hypoth{} \textbf{Questa appendix è il nucleo del paper tecnico EN.}
...
modelli sociali con formalism statistico & media-alta \\
\emph{Journal of Conflict Resolution} & Apprezza paper con predizioni
datate & alta \\
\emph{European Physical Journal B} & Modelli di synchronization
(Kuramoto) & alta \\
\end{longtable}}
}
...
Appendix}\label{note-epistemiche-di-chiusura-appendix}
...
hypothesis di linearità, regime quasi-stazionario \\
...
4 & Strategia B (diagonale scalata) & \Hypoth{} working hypothesis con
...
verificationre empiricamente nel paper EN \\
