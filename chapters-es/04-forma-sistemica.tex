\chapter{\texorpdfstring{La Forma Sistémica: Derivazione di
\(T_s^{(2),\text{sys}}\)}{La Forma Sistémica: Derivazione di T\_s\^{}\{(2),\textbackslash text\{sys\}\}}}\label{la-forma-sistemica-se derivazione-di-t_s2textsys}
...
coherencia de fase}, che amplifica l'antifragilidad collettiva e divide
l'involución collettiva. La differenza tra \(T_s^{(2)}\) local e
\(T_s^{(2),\text{sys}}\) sistemico misura quanto un sistema
\emph{spreca} per decoherencia interna.
\end{quote}
...
Il Cap.~3 ha se derivato \(T_s^{(2)}\) come quantità \emph{local} di un
attore con componenti perfettamente allineate (\(\Phi = 1\)). Il \S{}3.1bis
ha introdotto il parámetro de orden di Kuramoto
...
si comporta come un \textbf{moltiplicatore} per l'antifragilidad (la
coerenza estende il guadagno da stress all'intero sistema) e come un
\textbf{divisore} per l'involución (la decoherencia moltiplica gli
...
Il resto del capítulo se deriva, motiva, verificación e interpreta questa
asimmetria.
...
\item
  ...
  ciclo di legittimazione/decoherencia dell'acoplamiento;
\item
  una \textbf{frecuencia natural} \(\omega_j\) --- ritmo intrinseco
  dell'acoplamiento (cicli elettorali, di mercato, mediatici);
\item
  un \textbf{payoff instantáneo}
  \(\Pi_j(t) = M_{e,j} \cdot Q_{0,j} \cdot (1 - \delta_{R,j} z_{R,j})\)
  ereditato dalla legge micro della Topografia (\S{}3.1.2);
\item
  un'\textbf{antifragilidad local}
  \(A_j = \partial^2 \Pi_j / \partial \sigma^2\);
\item
  un \textbf{coefficiente di involución local}
...
donde \(K \geq 0\) è la \emph{forza di acoplamiento sistemica} (densità
mediatica, integrazione istituzionale, infrastruttura comunicativa).
L'ecuación si riduce, in termini del parámetro de orden \(\Phi\), a:

\[
\frac{d\theta_j}{dt} = \omega_j + K\Phi \sin(\psi - \theta_j)
\]

Esiste una umbral crítico \(K_c = 2/(\pi g(0))\), donde \(g(\omega)\) è
la distribuzione delle frequenze naturali, sotto cui \(\Phi = 0\)
(decoherencia completa) e sopra cui \(\Phi > 0\) (sincronización
parziale).

\Proved{} \textbf{Conse siguenza formal:} \(\Phi\) non è un parámetro
libero --- è determinato dalla densità infraestructural del sistema.
Sistemi con bassa \(K\) (frammentazione) hanno \(\Phi \to 0\) anche se
ciascun attore è perfettamente coerente con se stesso.



\section{Trasposizione di A: l'Antifragilidad
Sistemica}\label{trasposizione-di-a-lantifragilituxe0-sistemica}

\subsection{Argomento Strutturale}\label{argomento-estructural}
...
Questa forma è la \textbf{primero espansione di Taylor} di
...
\item
  ...
  recupera la media);
\item
  \(A_{\text{sys}}(\Phi = 0) = 0\) (la decoherencia piena annulla il
  trasferimento).
\end{itemize}
...
\(\Phi \to 1\) esistano effetti di saturación (rendimientos decrecientes)
...
rinviate a verificación empirica nel paper EN.



\section{Trasposizione di $\Omega$: l'Involuzione
Sistemica}\label{trasposizione-di-ux3c9-linvolución-sistemica}

\subsection{Argomento Strutturale}\label{argomento-estructural-1}

L'asimmetria con l'antifragilidad è essenziale. Mentre \(A\) produce
\emph{valore} che richiede coerenza per propagarsi, \(\Omega\) produce
\emph{spreco} che la decoherencia \textbf{amplifica strutturalmente}.
...
attore dissipa indipendientemente, e gli sprechi si \textbf{sommano senza
...
Condizioni al contorno verificaciónte:
...
\item
  \(\Omega_{\text{sys}}(\Phi \to 0) \to \infty\) (la decoherencia piena fa
...
\(T_s^{(2),\text{sys}}\)}{Derivazione di T\_s\^{}\{(2),\textbackslash text\{sys\}\}}}\label{se derivazione-di-t_s2textsys}
...
(legitimidad collettiva \(s_{iii}\), complejidad dialéctica media \(z\),
...
libro} (sección \emph{Apparato formal chiave}). La se derivazione qui
...
\(T_s^{(2),\text{sys}} \to \infty\) & Antifragilidad infinita salva il
sistema \Proved{} \\
...
la soglia di acoplamiento di Kuramoto, qualsiasi sistema collassa
\Proved{} \\
\end{longtable}}
}

\Proved{} \textbf{Risultato estructural.} Tutti i limiti hanno
...
fissati, la \textbf{umbral crítico di coerenza} \(\Phi^*\) è il valore
por lo cual \(T_s^{(2),\text{sys}}(\Phi^*) = 1\). Imponendo l'uguaglianza in
...
quadratica via formula classica è se derivata esplicitamente
nell'\textbf{Apéndice A \S{}A.7} ed è da preferire per sistemi in regime
di forte decoherencia (\(\Phi\) lontano da 1) --- caso paradigmatico, USA
del Cap.~7.

\Hypoth{} \textbf{Interpretazione.} Sistemi con \(\Phi(t) < \Phi^*\)
sono in \textbf{regime di transición de fase imminente} (nel senso del
...
In funzione di \(\Phi\) e dei parámetros locali, distinguiamo quattro
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Solitón metastabile} & n.a. (\(N=1\)) & molto alto & basso &
Musk \\
...
quantistico} non è il sistema con i parámetros locali migliori --- è
quello in cui \emph{la combinazione} di \(\Phi\) alto,
\(\langle A\rangle\) medio e \(\langle\Omega\rangle\) basso massimizza
\(T_s^{(2),\text{sys}}\). È una posizione estructural, non una proprietà
...
Nonostante \(\Phi\) alto e capacidad material enorme, la China ha
\(\langle\Omega\rangle\) molto alto (Neijuan estructural). Anche con la
divisione \(\langle\Omega\rangle/\Phi\) favorevole, l'involución resta
...
\(T_s^{(2),\text{sys}}\) chino sopra 1 ma con margine in erosione.
...
suo \(A\) individuale è eccezionale, ma non se puede aggregare. La sua
\emph{strategia di sopravvivenza} consiste nel restare uno (no
successione, no istituzionalizzazione) --- por lo tanto il suo \(T_s^{(2)}\) è
...
local modulata da una \textbf{coherencia de fase asimmetrica} ---
moltiplicativa per il valore (antifragilidad), divisoria per lo spreco
(involución) --- che traduce l'asimmetria termodinamica fondamentale
...
aperta nel \S{}3.1bis. Da qui in luego, il libro è applicato: il Cap.~5
...
\bottomrule\noalign{}
\endlastfoot
1 & L'hipótesis di campo medio di Kuramoto applicata al sistema sociale &
\Hypoth{} hipótesis operativa di scala \\
...
primero espansione consistente (alternative non lineari da testare) \\
4 & La forma \(\Omega_{\text{sys}} = \langle\Omega\rangle/\Phi\) &
\Hypoth{} primero espansione consistente (alternative non lineari da
testare) \\
...
\Proved{} verificacióndo analiticamente \\
7 & La soglia \(\Phi^*\) come indicatore precoce & \Hypoth{} hipótesis
operativa testabile \\
