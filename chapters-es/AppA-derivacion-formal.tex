\chapter{\texorpdfstring{Derivación Formal Completa di
\(T_s^{(2),\text{sys}}\)}{Derivación Formal Completa di T\_s\^{}\{(2),\textbackslash text\{sys\}\}}}\label{se derivazione-formal-completa-di-t_s2textsys}

\begin{quote}
\textbf{Scopo dell'apéndice.} Esplicitare ogni passaggio matematico
dalla legge micro-comunicativa \(\Pi_j(t+1)\) della Topografia (\S{}5.7)
fino alla forma sistemica \(T_s^{(2),\text{sys}}\) del Cap.~4,
dichiarando \emph{tutte} le hipótesis operative e identificando i punti in
cui la se derivazione ammette alternative. L'apéndice è progettata per
essere autonomamente verificaciónbile da un lettore con background in fisica
...
\((\mathcal{N}, \{\Pi_j\}, \mathcal{K})\) donde:
...
Definiamo il \textbf{parámetro de orden complesso}:

\[
\Phi(t) e^{i\psi(t)} := \frac{1}{N} \sum_{j=1}^{N} e^{i\theta_j(t)}, \quad \Phi \in [0, 1], \quad \psi \in [0, 2\pi)
\]

\Proved{} \textbf{Riduzione di campo medio.} L'ecuación di Kuramoto si
...
(passaggio standard, vedi Strogatz 2000 per la se derivazione).
...
di decoherencia completa); -
\(K > K_c \Rightarrow \Phi \to \Phi_\infty(K) > 0\) asintoticamente
(regime di sincronización parziale); - \(\Phi_\infty(K) \to 1\) per
\(K \to \infty\) (sincronización piena).
...
magnitud local \(X_j\) definita su \(\mathcal{N}\), la \emph{quantità
sistemica se derivata} è:

\[
\langle X \rangle_\Phi := \Phi \cdot \frac{1}{N} \sum_{j=1}^{N} X_j = \Phi \cdot \langle X \rangle
\]

donde \(\langle X \rangle := N^{-1} \sum_j X_j\) è la media aritmetica
local.
...
misura quanto \(X\) è \emph{perduto per decoherencia}.
...
Taylor).} L'antifragilidad sistemica \(A_{\text{sys}}\) è la più semplice
...
amplificazione inversa).} L'involución sistemica
...
se \(\alpha\) tal que \(1-e^{-\alpha} = 1\) (es decir,
\(\alpha \to \infty\), recupera lineare) & saturación per \(\Phi\)
alto \\
\(A_{\text{sys}} = \Phi^\beta \langle A\rangle\) & \Proved{} per
qualunque \(\beta > 0\) & \(\beta < 1\): rendimientos decrecientes;
...
\(\Omega_{\text{sys}} = \langle\Omega\rangle/\Phi\) sono la \emph{primero
...
\(\langle \cdot \rangle_\Phi\) dado que sono parámetros di campo esogeno o
...
\item
  denominatore: la decoherencia ha due componenti additive --- una
  \emph{proporzionale a \(\Phi\)} (volatilità modulata dalla coerenza) e
  una \emph{indipendiente da \(\Phi\)}
  (\(\sigma \varepsilon_{\text{dis}} \langle\Omega\rangle\),
  l'involución estructural).
\end{itemize}
...
\Proved{} \textbf{Tabla di verificación completa.}
...
& Antifragilidad infinita \(\Rightarrow\) stabilità infinita (caso
teorico) \Proved{} \\
...
& Sotto soglia di acoplamiento \(\Rightarrow\) colapso sistemico
inevitabile \Proved{} \\
\end{longtable}}
}
...
\Hypoth{} \textbf{Definizione operativa.} La \emph{umbral crítico di
coerenza} \(\Phi^*\) è il valore di \(\Phi\) por lo cual
\(T_s^{(2),\text{sys}} = 1\):
...
per sistemi in regime di forte decoherencia (USA in Cap.~7).

\Proved{} \textbf{Limite consistente.} Per
\(\langle\Omega\rangle \to 0\) (\(c \to 0\)), \(\Phi^* \to 0\) se
\(b > 0\) --- recupero della primero formulazione lineare per sistemi
senza involución.
...
instabile} con respecto a perturbazioni in \(\Phi\).
...
\Hypoth{} \textbf{Conse siguenza dinamica.} Perturbazioni
\(\Phi^* + \epsilon\) con \(\epsilon < 0\) portano
\(T_s^{(2),\text{sys}} < 1\) --- il sistema si auto-rinforza nella
decoherencia mediante R3 (\(\Phi\)-\(\varepsilon_{\text{dis}}\) negativo,
Cap.~5.1) e R2 (\(\Phi\)-\(\Omega\) negativo). Perturbazioni
\(\Phi^* + \epsilon\) con \(\epsilon > 0\) portano
\(T_s^{(2),\text{sys}} > 1\) --- il sistema si auto-rinforza nella
coerenza per gli stessi meccanismi.

\Proved{} \textbf{Risultato estructural.} \(\boldsymbol{p}^*\) è un
\emph{separatore di destini}: piccole perturbazioni in \(\Phi\)
producono, a través de le sei relazioni R1-R6, traiettorie che divergono
in \(\mathbb{V}^{(4)}\). Questo è la base formal del concetto di
\emph{transición de fase} del Cap.~3.5.3 e della \emph{bifurcación}
del Cap.~9.5.



\section{Note sulle Ipotesi
Implicite}\label{note-sulle-hipótesis-implicite}

\Conj{} \textbf{Ipotesi A4 (separazione di scala temporale).} I
parámetros esogeni \((s_{iii}, z, \delta, \sigma)\) variano su scale
temporali molto più lunghe (decennali per \(s_{iii}, z, \delta\); ciclo
geopolitico per \(\sigma\)) con respecto alle variables di
\(\mathbb{V}^{(4)}\) (\(\Phi\) stagionale, \(A\) post-stress, \(\Omega\)
trimestrale, \(\varepsilon_{\text{dis}}\) semestrale). Questa
separazione di scala giustifica trattare i parámetros come quasi-costanti
per finestre di observación di 5-10 anni.
...
\Proved{} \textbf{Conse siguenza metodologica.} Le tre hipótesis A4-A6 sono
...
\Proved{} \textbf{Risultato centrale.} La se derivazione formal completa
di \(T_s^{(2),\text{sys}}\) dal campo dei payoff topografici
\(\{\Pi_j(t)\}\) richiede sei hipótesis esplicite (A1-A6, di cui due come
...
L'equilibrio \(\boldsymbol{p}^* \in \mathcal{S}\) è instabile con respecto a
\(\Phi\), giustificando formalmente la nozione di \emph{bifurcación
...
(sei hipótesis), \emph{consistente} (sei limiti verificaciónti), \emph{aperto}
...
\emph{Apéndice tecnica. Pre-requisiti: fisica statistica (Strogatz
