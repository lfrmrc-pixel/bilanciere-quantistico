\chapter{Predicciones Arriesgadas Fechadas: Falsificabilità
Pubblica}\label{predizioni-rischiose-datate-falsificabilituxe0-pubblica}

\begin{quote}
\textbf{Argumento central:} una teoria sociale che non si espone alla
falsificación \emph{datata} è indistinguibile da una narrazione. Questo
capítulo aggrega le sedici predicciones arriesgadas della DEQ$^2$ --- tredici
se derivate dai Cap.~7-9 sui tre attori sistemici (cinque USA, quattro
China, quattro Brasil), tre se derivate dal Cap.~10 sull'aggregato globale
...
ciascuna predizione è además specificato \emph{cosa accadrebbe alla
teoria se fallisse}: la falsificación di alcune predizioni è gestibile
come corrección local, quella di altre forzerebbe la riformulazione
estructural dell'apparato.
\end{quote}



\section{Criterios DEQ$^2$ per una Predizione
...
\item
  ...
  verificaciónbile da terzi indipendenti;
\item
  \textbf{Probabilità soggettiva esplicita}:
  \(\Pr(\mathcal{P} \text{ vera}) = p\) è dichiarata, \(p \in (0, 1)\);
\item
  \textbf{Conse siguenza teorica}: è dichiarato cosa accade alla DEQ$^2$ se
  \(\mathcal{P}\) fallisce --- \emph{corrección local},
  \emph{ajuste paramétrico}, oppure \emph{riformulazione
  estructural}.
\end{enumerate}
...
aggiornamento esplicito delle probabilità posteriori sui parámetros della
...
\subsection{USA --- Hegemón Decoherente (Cap.
...
\(0{,}70\) & local (parámetro \(\Phi\)) \\
\textbf{U2} & Quota dollaro nelle riserve IMF COFER \(< 55\%\) & Q4 2027
& \(0{,}55\) & IMF COFER trimestrale & \(0{,}55\) & local (parámetro
\(\sigma\) esogena) \\
...
\(0{,}55\) & parámetrosca (\(\Omega\) acuta vs cronica) \\
\textbf{U5} & Correlazione Musk visibility--polarizzazione USA
\(r > 0{,}3\) & calibrazione 2022-2026 & \(r = 0{,}3\) & dataset
compilato (Cap.~7) & \(0{,}65\) & local (modello
\(\zeta_{\text{Sol}}\)) \\
\end{longtable}}
}

\subsection{China --- Rígido Sincronizado (Cap.
...
\textbf{C4} & PPI chino in territorio negativo \(\geq 5\) anni cumulati
& Q4 2027 & \(5\) anni & NBS PPI mensile & \(0{,}70\) & local
(anti-involution efficacia) \\
\end{longtable}}
}

\subsection{Brasil --- Balanza Cuántica (Cap.
...
meta-stima soggettiva & ajuste paramétrico al cambiare
evidenze \\
...
condicional & local (operatività iniziativa) \\
\textbf{B3} & Se A/B vince: V-Dem Brasil Liberal Democracy Index
\(> 0{,}65\) & report 2028 (anno 2027) & \(0{,}65\) &
\url{https://v-dem.net} & \(0{,}65\) condicional & local (\(\Phi\)
Brasil) \\
...
ufficiali & \(0{,}60\) condicional & estructural (la
...
predizione local aggiusterebbe parámetros specifici; il fallimento di
una predizione estructural costringerebbe a ripensare l'apparato
corrispondente.
...
conferma di G1 con falsificación di G2 (\(\Pr \approx 0{,}25\))
...
2026}\label{la-predizione-meta-lelezione-brasileña-del-4-ottobre-2026}
...
actualización bayesiana della probabilità del Mondo M1/M2 \\
...
Falsificazione}\label{protocollo-di-pubblicazione-e-falsificación}

\Hypoth{} \textbf{Pre-registrazione.} Le sedici predizioni di questo
capítulo sono \emph{pre-registrate} alla data di pubblicazione del libro
(target 2026 Q3 --- KDP IT/EN/ES/PT). La pre-registro consiste
...
\tightlist
\item
  \textbf{Tabla delle predizioni} (sezioni 11.1, 11.2) con tutti i
  campi (1)-(5);
\item
  ...
  \tightlist
  \item
    file CSV con tutte le predizioni e i loro stati (pendiente / vera /
    falsa);
  \item
    script Python di calcolo automatico delle predizioni quantitative
    dai dataset pubblici;
  \item
    log dei \emph{checkluegont} trimestrali con re-stima probabilità
    soggettive;
  \end{itemize}
\item
  ...
  \item
    actualización bayesiana delle probabilità;
  \item
    eventuali correzioni alla teoria (locali/parámetrosche/strutturali).
  \end{itemize}
\end{enumerate}

\Proved{} \textbf{Conse siguenza editoriale.} Il libro non è
...
della Topografia Ap.~A: produrre segnali \((x, y, z)\) alti su
\emph{tutti} gli assi --- incluso \(z\) (complejidad dialéctica) mediante
l'esposizione esplicita alla falsificación.
...
Predizione
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Fallimento $\to$ Conse siguenza per DEQ$^2$
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
del Cap.~6.6 va riformulato. Locale-parámetrosca. \\
C1 & Deflazione finisce primero: anti-involution funziona. Aggiustamento
\(\Omega^{\text{Cina}}\). Locale. \\
C2 & Nessun bloqueo malgrado condizioni soddisfatte: hipótesis
\emph{Taiwan come valvola} va rivista. \textbf{Strutturale}. \\
C3 & Yuan oltre \(5\%\) entro 2030: la dedollarizzazione converge sullo
yuan, contro la tesi Cap.~8.6. Locale-parámetrosca. \\
C4 & PPI positivo primero del 2027: anti-involution efficace.
...
B4 & (condicional a B1 falsa) fallimento --- Brasil resiste sotto
...
\textbf{Strutturale}. \\
G2 & Brasil fallisce \(C_{\text{DEQ}}^2 \geq 1\) malgrado A/B: la
definición di \(\Phi\)-engine è insufficiente. \textbf{Strutturale}. \\
G3 & China mantiene \(d^{\text{oss}} \approx d^{\text{spont}}\): la
categoria \emph{Rígido Sincronizado} non descrive la China come
...
U5). Il fallimento di una predizione estructural impone
...
condicional debole): \(\Pr \approx (1-0{,}55)^5 \approx 0{,}018\) ---
...
invalidare l'apparato; il libro è por lo tanto un \emph{pre-registered
prediction set} il cui valore epistemico cresce o diminuisce in
proporzione alla performance verificaciónbile della teoria.
...
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Tipo di falsificación
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Cap.~7 USA & 5 & \(0{,}60\) & 4 locali, 1 parámetrosca/meta (U5) \\
Cap.~8 China & 4 & \(0{,}60\) & 3 locali, 1 estructural (C2) \\
Cap.~9 Brasil & 4 & meta + condizionali & 2 condizionali, 1 estructural
(B4), 1 meta (B1) \\
...
\Hypoth{} \textbf{Tesi conclusiva.} Le predizioni di questo capítulo
sono il \emph{contenuto empirico futuro} della DEQ$^2$. La Parte V del
libro (Cap.~12-13) costruirà en cambio il \emph{contenuto normativo}: dato
...
\bottomrule\noalign{}
\endlastfoot
1 & Cinque criteri DEQ$^2$ per predicción arriesgada & \Hypoth{}
metodologia \\
...
se derivazione coerente \\
4 & B1 come predizione meta che condiziona la cascata & \Hypoth{}
architettura logica \\
5 & Protocollo di pre-registro e repository GitHub & \Hypoth{}
...
\Conj{} stima sotto indipendenza condicional \\
...
\section{Chiusura della Parte IV e Apertura de la Parte
...
Cap.~11 ha aggregato le 16 predicciones arriesgadas datate con criteri
popperiani operativi e protocollo di falsificación pubblica.
...
\item
  ...
  50+ paesi), inviti a falsificación.
\end{itemize}
