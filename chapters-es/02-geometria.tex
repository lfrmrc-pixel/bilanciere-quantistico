...
legitimidad); la sua interazione strategica con altri attori se sigue un
payoff la cui forma è identica a quella del singolo acoplamiento
...
Il Cap.~1 ha presentato la Topografia come microfundación comunicativa.
...
le due strutture sono \emph{unificabili} a través de l'operatore di
Kuramoto. Per seguire quella demostración, el lector ha bisogno
...
\tightlist
\item
  \(s_{i,1} = s_i\) = \textbf{cohesión interna} (integrazione politica,
  omogeneità decisionale, controllo del territorio);
\item
  \(s_{i,2} = s_{ii}\) = \textbf{capacidad material} (PIL, popolazione,
  tecnologia, energia, forze armate);
\item
  \(s_{i,3} = s_{iii}\) = \textbf{legitimidad ético-discursiva} (soft
  power, legitimidad internazionale, qualità del discorso pubblico).
\end{itemize}
...
(sottovarietà di codimensione 1) nello spazio estructural
quadridimensionale.
...
\(Q_0\) della Topografia, applicata agli stati egemonici en cambio che alle
...
donde \(H_i\) è l'\textbf{impronta sistemica} (la ``massa egemonica'' ---
analogo macro di \(M_e\)) e \(\delta_i\) il factor de descuento. Il
termine \((1 - \delta_i s_{i,3})\) è il \textbf{factor de erosión di
legitimidad}: la legitimidad conquistata si consuma nell'esercizio del
potere.

\Proved{} \textbf{Identità formal con Topografia (verificaciónta nel Cap.~3
\S{}3.1.3).} Il payoff egemonico è strutturalmente la stessa ecuación
della función predictiva della Topografia
...
\bottomrule\noalign{}
\endlastfoot
\(M_e\) (masa comunicativa) & \(H_i\) (impronta sistemica) \\
\(Q_0\) (calidad ética) & \(Q_i\) (qualità strategica) \\
\(\delta_R\) (sconto del ricevente) & \(\delta_i\) (sconto
dell'attore) \\
\(z_R\) (complessità del ricevente) & \(s_{i,3}\) (legitimidad erosa
nell'uso) \\
\end{longtable}}
}
...
(anziché conflitto totale) è sostenibile come equilibrio de Nash
...
invarianza de escala --- la stessa struttura governa cooperazione
comunicativa micro e cooperazione egemonica macro. La DEQ$^2$ la dimostra
come teorema estructural (Cap.~3).
...
entrano in regime di non-cooperazione estructural, indipendientemente
...
Il \emph{Memoriale Tecnico DEQ} (2026-04-23) se deriva, dalle ecuaciones del
...
\tightlist
\item
  \(s_{iii}\) = legitimidad ético-discursiva del sistema;
\item
...
\item
  \(\sigma\) = volatilidad estocástica ambientale (estensione EDSD, \S{}2.5);
\item
  \(\varepsilon_{\text{dis}}\) = coeficiente de desvinculación moral
  (Bandura 1999, Milgram 1974).
\end{itemize}

\Proved{} \textbf{Lettura.} Numeratore: forze di coerenza. Denominatore:
forze di decoherencia. La umbral crítico è \(T_s^{(1)} = 1\). Sopra,
sistema stabile; sotto, transición de fase imminente.
...
endogeno (involución). Due sistemi con stesso \(T_s^{(1)}\) possono
...
donde \(W_k(t)\) è un processo di Wiener e \(\sigma_k\) è la volatilità
...
\Hypoth{} \textbf{Conse siguenza per la DEQ$^2$.} L'antifragilidad \(A\) del
...
(invarianza de escala) & Cap.~3 \S{}3.1.3 \\
...
microfundación. La DEQ$^2$ (Cap.~3-4) compie il passo che unifica i due
...
\bottomrule\noalign{}
\endlastfoot
1 & Spazio \(\mathcal{H}\) degli stati egemonici & \Proved{} definición
operativa & Geometria \S{}2 \\
2 & Forma di \(Q_i\) (identica a \(Q_0\) Topografia) & \Proved{}
verificacióndo & Geometria \S{}2 / DEQ$^2$ Cap.~3 \\
3 & Payoff \(\Pi_i\) identico a $\Pi$ Topografia & \Proved{} verificacióndo
(Cap.~3) & Geometria \S{}3 / DEQ$^2$ Cap.~3 \\
4 & Teorema Folk \(\delta^* = 0{,}50\) & \Proved{} se derivato & Geometria
\S{}3 \\
5 & \(T_s^{(1)}\) come soglia composita & \Hypoth{} hipótesis operativa &
Memoriale DEQ \\
