\chapter{\texorpdfstring{Calibración Empírica de
\(\Sigma^{\text{geo}}\) a Partire da
\(\Sigma^{\text{comm}}\)}{Calibración Empírica de \textbackslash Sigma\^{}\{\textbackslash text\{geo\}\} a Partire da \textbackslash Sigma\^{}\{\textbackslash text\{comm\}\}}}\label{calibrazione-empirica-di-sigmatextgeo-a-partire-da-sigmatextcomm}

\begin{quote}
\textbf{Argumento central:} la matrice di covarianza degli stati
egemonici \((s_i, s_{ii}, s_{iii})\) può essere stimata, in primero
...
sistema egemonico --- necessaria, por ejemplo, per definire \(\sigma\)
...
pubblicazione precedente. La presente apéndice propone una procedura di
...
\subsection{Definizione}\label{definición}

Sia \(\mathbf{c}_j = (x_j, y_j, z_j)^T\) il vettore comunicativo
dell'acoplamiento elementare \(j\) all'interno di un sistema sociale, e
...
primero approssimazione --- non lineare in regimi estremi (colapso,
transición de fase) ma adeguata in regimi quasi-stazionari, che è il
...
identificando \(J\), obtenemos \(\Sigma^{\text{geo}}\) senza necessità
...
Identitaria}\label{strategia-a-hipótesis-nulla-identitaria}
...
non-ortogonalità si conserva sotto aggregazione. È l'hipótesis più
...
\tightlist
\item
  \(\alpha_1\): peso della coherencia formal micro \(x\) nella coesione
  interna macro \(s_i\);
\item
  \(\alpha_2\): peso della precisión técnica micro \(y\) nella capacità
  materiale macro \(s_{ii}\);
\item
  \(\alpha_3\): peso della complejidad dialéctica micro \(z\) nella
  legitimidad macro \(s_{iii}\).
\end{itemize}
...
Ipotesi più generale: \(J\) è \(3 \times 3\) piena, \(9\) parámetros
liberi. Permette di catturare effetti come ``il discorso tecnico genera
capacidad material ma anche coesione'' (\(J_{12} > 0\)) o ``la
complejidad dialéctica produce legitimidad ma erode coesione''
(\(J_{31} > 0,\ J_{11}\) ridotto).

\Conj{} \textbf{Limite.} \(9\) parámetros richiedono almeno 9 punti dati
...
donde la base dati può essere costruita.
...
\bottomrule\noalign{}
\endlastfoot
\(s_i\) --- cohesión interna & World Bank Government Effectiveness &
Polity IV, V-Dem Liberal Democracy & WB / Polity / V-Dem \\
\(s_{ii}\) --- capacidad material & PIL nominale $\times$ Economic Complexity
Index & Spesa militare, brevetti, energia & WB / IMF / OEC \\
\(s_{iii}\) --- legitimidad ético-discursiva & Brand Finance Soft Power
...
standardizzate per coherencia formal, precisión técnica, complessità
dialettica.
...
materiale) è il più alto: la precisión técnica del discorso si traduce
\emph{più che proporzionalmente} in capacidad material macroscopica.
...
\section{Conse siguenze per la DEQ$^2$}\label{conse siguenze-per-la-dequxb2}
...
Questo è il livello base di volatilità aggregata de donde le predizioni
del Cap.~11 partono.

\subsection{\texorpdfstring{Calibrazione di
\(\Phi\)}{Calibrazione di \textbackslash Phi}}\label{calibrazione-di-phi}

\Hypoth{} La densità di acoplamiento \(K\) del Cap.~4 può essere
...
(cooperazione interna effettiva), e por lo tanto sono più probabilmente sopra
...
dovrebbe avere \(R^2 \geq 0{,}75\) con respecto alla \(\Sigma^{\text{geo}}\)
...
\Hypoth{} \textbf{Questa apéndice è il nucleo del paper tecnico EN.}
...
datate & alta \\
\emph{European Physical Journal B} & Modelli di sincronización
(Kuramoto) & alta \\
\end{longtable}}
}
...
Apéndice}\label{note-epistemiche-di-chiusura-apéndice}
...
hipótesis di linearità, regime quasi-stazionario \\
...
4 & Strategia B (diagonale scalata) & \Hypoth{} hipótesis operativa con
...
verificaciónre empiricamente nel paper EN \\
