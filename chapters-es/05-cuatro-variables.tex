...
\(\varepsilon_{\text{dis}}\) non sono parámetros indipendenti che entrano
in \(T_s^{(2),\text{sys}}\) come ingredienti di una ricetta --- sono
\textbf{componenti di uno spazio estructural quadridimensionale
\(\mathbb{V}^{(4)}\)} in cui ogni sistema sociale occupa un punto, e in
cui esistono \emph{sei relazioni di acoplamiento bivariate} che
...
attori in \(\mathbb{V}^{(4)}\) con respecto a tale ipersurface.
\end{quote}



\section{\texorpdfstring{Lo Spazio Strutturale
\(\mathbb{V}^{(4)}\)}{Lo Spazio Strutturale \textbackslash mathbb\{V\}\^{}\{(4)\}}}\label{lo-spazio-estructural-mathbbv4}

\Hypoth{} \textbf{Definizione.} Sia
\(\mathbb{V}^{(4)} := [0, 1] \times \mathbb{R} \times \mathbb{R}_{\geq 0} \times [0, 1]\)
lo spazio estructural DEQ$^2$, parámetroszzato da:
...
\bottomrule\noalign{}
\endlastfoot
\(\Phi\) & parámetro de orden di Kuramoto & \([0, 1]\) & Cap.~4 ---
sincronización di fase del campo \(\Pi\) \\
\(A\) & antifragilidad & \(\mathbb{R}\) (sub-dominio operativo
\([-1, +\infty)\)) & Cap.~3.3.1 --- \(\partial^2\Pi/\partial\sigma^2\)
(Taleb formalizzato) \\
\(\Omega\) & involución & \(\mathbb{R}_{\geq 0}\) & Cap.~3.3.2 ---
...
\Proved{} \textbf{Conse siguenza formal.} Ogni sistema sociale è
...
donde i parámetros ereditati dalla Geometria/Topografia
\((s_{iii}, z, \delta, \sigma)\) sono trattati come \emph{campi esogeni}
(lentamente variables con respecto alla scala temporale di
\(\boldsymbol{p}\)).
...
\item
  ...
  --- \textbf{piano della decoherencia-spreco}.
\end{itemize}
...
Bivariate}\label{le-sei-relazioni-di-acoplamiento-bivariate}

\Hypoth{} \textbf{Tesi.} Le quattro variables \emph{non sono
indipendenti}. Esistono \(\binom{4}{2} = 6\) relazioni di acoplamiento
bivariate, ciascuna con un segno e una giustificazione estructural. Esse
...
Antifragilidad trasforma volatilità in opzionalità \(\Rightarrow\) riduce
dissipazione estructural & USA: \(A_{\text{sys}} < 0\) (hiperextensión)
...
\Proved{} \textbf{Risultato estructural.} Quattro relazioni hanno segno
...
energetico-produttivo}: \(A\) ne misura la convessità con respecto allo
shock, \(\Omega\) ne misura la dissipazione interna. Si spiega así
...
\(\zeta(t)\) \\
\textbf{Hegemón Decoherente (USA)} & \(0{,}25\)-\(0{,}35\) (V-Dem
\(0{,}57\), Pew, Brand Finance) & \(-0{,}3\) (sistemico, da
hiperextensión) & \(0{,}8\)-\(1{,}2\) (polarizzazione, tariffe
...
\(R_c\) effettivo) & \(+0{,}4\) (resiliencia 2018-2023 confermata) &
...
\Hypoth{} \textbf{Lettura estructural.} Solo il Brasil ha il punto
\(\boldsymbol{p}(t)\) \textbf{interno} alla regione
\(T_s^{(2),\text{sys}} > 1\) in modo non degenere. USA, China e Solitón
sono in tre forme distinte di patologia estructural:

\begin{itemize}
\tightlist
\item
  USA: punto \emph{interno} alla regione \(T_s^{(2),\text{sys}} < 1\)
  (decoherencia distribuita stabile);
\item
  China: punto \emph{sul bordo} della regione critica, mantenuto dalla
  forzatura \(K^{\text{imposto}}\) (instabile a perturbazioni di \(K\));
\item
  Solitón: punto \emph{fuori dominio} sistemico (\(N=1\), \(\Phi\)
  degenere).
\end{itemize}
...
\Hypoth{} \textbf{Costruzione.} L'ecuación
...
transición de fase}. Esplicitando dalla forma del Cap.~4:
...
lati si ottiene un'ecuación quadratica in \(\Phi^*\):
...
quadratica è se derivata in \textbf{Apéndice A \S{}A.7} ed è preferibile per
sistemi in regime di forte decoherencia, donde \(\Phi\) è lontano da 1).
...
\(A > 0\) & senza involución, solo sistemi \emph{fragili} possono
attraversare la soglia \\
Sezione \(A = 0\) &
\(\Phi^* = (\sigma\varepsilon\Omega - s_{iii}z\delta) / (-\sigma\varepsilon)\)
& senza antifragilidad, \(\Phi^*\) cresce linearmente con \(\Omega\) \\
...
\item
  \(\mathcal{R}_- := \{\boldsymbol{p} : T_s^{(2),\text{sys}}(\boldsymbol{p}) < 1\}\)
  --- regione di \textbf{transición de fase imminente} (Cap.~3.5.3,
  \(\Delta t > \Delta t^*\)).
\end{itemize}
...
secondo un sistema di ecuaciones differenziali accoppiate, con una
...
\tightlist
\item
  \(K(t)\) è la forza di acoplamiento di Kuramoto del Cap.~4 (esogena,
  dipende da infrastruttura comunicativa e istituzionale);
\item
  \(\zeta(t)\) è la sorgente esogena di disordine (Cap.~6.6: il Solitón
...
\item
  ...
  l'auto-rinforzo dell'involución quando il payoff aggregato scende
  sotto il livello di soglia \(\Pi^*\);
\item
  \(Q_0(t)\) è la calidad ética del campo discorsivo (eredità Topografia
  \S{}5.2);
\item
  \(\eta_X(t)\) sono rumori stocastici a media nulla.
\end{itemize}

\Proved{} \textbf{Conse siguenza interpretativa.} Le sei relazioni R1-R6 di
\$\S\$5.1 emergono come \emph{equilibri quasi-statici} del sistema
dinamico: i segni dei coefficienti \(\beta, \mu, \alpha\) realizzano
R1-R6 nelle ecuaciones evolutive. Per ejemplo,
...
estructural è che fornisce uno \emph{schema di pensiero} per organizzare
gli indicatori monitorabili del Cap.~10.

\Hypoth{} \textbf{Applicazione ai quattro attori (verificación
qualitativa).}
...
\textbf{Solitón} & non applicabile sistemicamente; agisce come
\(\zeta\) esogeno & influenza altri sistemi via acoplamiento esogeno
(Cap.~6.6, 7.4) \\
\end{longtable}}
}
...
\Proved{} \textbf{Conse siguenza strategica della Parte V.} Le strategie
...
in \(\mathcal{R}_+\) con margine di sicurezza} con respecto a
\(\mathcal{S}\).



\section{Indicatori Osservabili e Protocollo di
Misurazione}\label{indicatori-osservabili-e-protocollo-di-medición}

\Proved{} \textbf{Mappa indicatori-variables} (preparazione Cap.~10):
...
\(A\) & resiliencia 2008-2009 + COVID-2020 + 2022-2023 (3 stress tests) &
...
paper EN (Apéndice B). I quattro attori del libro sono casi-studio
...
spazio estructural in cui ogni sistema sociale è un punto, vincolato da
sei relazioni di acoplamiento bivariate (R1-R6) e dotato di una
superficie critica \(\mathcal{S}\) che separa i regimi di stabilità e di
transición de fase imminente; le strategie politiche del libro saranno
...
antifragili \\
Mappa indicatori-variables & Operativizzazione & Cap.~10 dashboard \(+\)
Apéndice C \\
\end{longtable}}
}

\Hypoth{} \textbf{Tesi conclusiva.} Il Cap.~4 ha se derivato
\(T_s^{(2),\text{sys}}\) come \emph{funzione scalare} delle quattro
variables strutturali. Il Cap.~5 mostra che quelle variables sono
...
unificato è completo, la Parte III ne ha verificacióndo l'applicabilità sui
quattro attori paradigmatici, e la Parte IV può procedere alla
\emph{operativizzazione} mediante dashboard, scenari quantizzati e
...
\bottomrule\noalign{}
\endlastfoot
1 & Definizione di \(\mathbb{V}^{(4)}\) come spazio estructural &
\Hypoth{} costruzione formal \\
2 & Sei relazioni R1-R6 con segni e meccanismi & \Hypoth{} quadro
analitico, segni verificaciónti nei Cap.~6-9 \\
...
estructural, usata operativamente nel Cap.~7 \\
5 & Mappa quattro attori in \(\mathbb{V}^{(4)}\) con stime numeriche &
\Conj{} stime parámetrosche con intervalli di confidenza ampi \\
6 & Superficie critica \(\mathcal{S}\) con forma esplicita
\(\Phi^*(A, \Omega, \varepsilon)\) & \Proved{} se derivata analiticamente
da Cap.~4 \\
7 & Topologia di \(\mathbb{V}^{(4)}\) in \(\mathcal{R}_+\) e
\(\mathcal{R}_-\) & \Proved{} conse siguenza diretta di
\(T_s^{(2),\text{sys}} = 1\) \\
...
10 & Mappa indicatori-variables per dashboard & \Proved{} se derivata da
fonti citate nei Cap.~7-9 \\
11 & Validazione interna: posizioni
\(\boldsymbol{p}_{\text{USA, Cina, Brasile}}\) consistenti con tutte
R1-R6 & \Proved{} verificación non banale dell'apparato \\
\end{longtable}}
}



\section{Interpretaci\'{o}n Evolutiva de las Cuatro Variables}\label{interpretazione-evolutiva-quattro-variables}

\Hypoth{} \textbf{Las cuatro variables como firma evolutiva.} Cada
variable de \(\mathbb{V}^{(4)}\) tiene una definici\'{o}n
fenomenol\'{o}gica (Cap.~5) y una \emph{microfundaci\'{o}n causal
evolutiva} (§D.4, marco CPGG). Las dos lecturas no se sustituyen entre
s\'{i}: se estratifican, ofreciendo al analista herramientas de
diagn\'{o}stico a distintos niveles de abstracci\'{o}n.
...
Variable
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Definici\'{o}n DEQ$^2$ (Cap.~3--5)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Microfundaci\'{o}n evolutiva (\Hypoth{})
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\(\Phi\) & Par\'{a}metro de orden de Kuramoto: alineaci\'{o}n de fase del
campo \(\Pi\) & Firma sist\'{e}mica del r\'{e}gimen \(rB > C\) (Hamilton
1964): \(\Phi > 0\) cuando los mecanismos cooperativos prevalecen sobre la
extracci\'{o}n \\ \addlinespace
\(A\) & Convexidad de \(\Pi\) respecto a la volatilidad \(\sigma\) &
Robustez de la condici\'{o}n \(rB > C\) ante perturbaciones: \(A > 0\)
indica que el sistema gana cooperaci\'{o}n de los choques en lugar de
perderla \\ \addlinespace
\(\Omega\) & Raz\'{o}n energ\'{i}a disipada/\'{u}til (Neijuan) &
Coeficiente derivado del CPGG: \(\Omega \approx \phi\eta r/(r-1)\); mide
la cantidad de extracci\'{o}n que la sub-red \(\mathcal{E}\) ejerce sobre
el campo cooperativo \\ \addlinespace
\(\varepsilon_{\text{dis}}\) & Coeficiente de desvinculaci\'{o}n moral
(Bandura/Milgram) & Degradaci\'{o}n end\'{o}gena de las instituciones que
mantienen \(rB > C\): aumenta con \(\phi > \phi^*\) a trav\'{e}s de la
sigmoide \(\mathcal{S}(\mu[\phi - \phi^*])\) \\
\end{longtable}}
}

\Proved{} \textbf{Lectura integrada.} Un sistema con \(\Phi\) alto,
\(A\) positivo, \(\Omega\) bajo y \(\varepsilon_{\text{dis}}\) contenido
es un sistema en el que: (i) la condici\'{o}n \(rB > C\) se cumple para
la mayor\'{i}a de los sub-actores; (ii) el sistema es robusto ante los
choques cooperativos; (iii) la fracci\'{o}n extractiva \(\phi\) est\'{a}
por debajo del umbral cr\'{i}tico \(\phi^*\); (iv) las instituciones
normativas no est\'{a}n degradadas. Esta es exactamente la posici\'{o}n
de la \textbf{Balanza Cu\'{a}ntica} en el Cap.~9 --- y ahora sabemos
\emph{por qu\'{e}} esa posici\'{o}n es estable, no solo \emph{que} lo es.

\Hypoth{} \textbf{El sistema como commons de Ostrom a escala
geopol\'{i}tica (\Hypoth{}).} El problema normativo del Cap.~12 se
traduce, en la lectura evolutiva, en dise\~{n}ar las condiciones
institucionales --- los ocho principios de Ostrom (1990) escalados a la
geopolítica --- que mantienen \(\phi < \phi^*\) y por lo tanto
\(\Phi > 0\) a largo plazo. El DEQ$^2$ ofrece el diagn\'{o}stico; el
aparato evolutivo ofrece el \emph{por qu\'{e}} ese diagn\'{o}stico es
necesario.



\section{Chiusura della Parte II e Apertura de la Parte
IV}\label{chiusura-della-parte-ii-e-apertura-della-parte-iv}

La Parte II del libro è ora completa: Cap.~3 ha se derivato \(T_s^{(2)}\)
local, Cap.~4 ha se derivato \(T_s^{(2),\text{sys}}\) con asimmetria
\(\Phi\)-moltiplica-\(A\) vs \(\Phi\)-divide-\(\Omega\), Cap.~5 ha
consolidato le quattro variables come spazio estructural
\(\mathbb{V}^{(4)}\) con sei relazioni di acoplamiento. La Parte III ha
...
\item
  ...
  falsificación pubblica} della DEQ$^2$, con date, soglie numeriche, fonti
  dati.
\end{itemize}
