\chapter{O Hegemon Decoerente (EUA): Iper-estensione senza
Coerenza}\label{legemone-decoerente-usa-hiperextensão-senza-coerenza}

\begin{quote}
\textbf{Argumento central:} os Estados Unidos del secondo mandato Trump
esibiscono il pattern formale dell'\emph{egemone decoerente}: massima
capacidade material \(s_{ii}\) residua, acoplamento sistemico interno
\(K > K_c\), ma parâmetro de ordem \(\Phi \to 0\) per effetto combinato
di hiperextensão esecutiva, polarização afetiva, declino di
\(s_{iii}\) e azione di sorgenti esogene di disordine \(\zeta(t)\) ---
fra cui il Soliton Musk del Cap.~6. Il sistema è formalmente in regime
sub-critico apesar de il superamento della soglia di acoplamento.
\end{quote}



\section{Definição Estrutural dell'Egemone
Decoerente}\label{definição-estrutural-dellegemone-decoerente}

\Hypoth{} \textbf{Definizione (Hegemon Decoerente DEQ$^2$).} Un attore
\(e\) è detto \emph{Hegemon Decoerente} se soddisfa simultaneamente:
...
\item
  ...
  (capacidade material residua sopra la media sistemica per almeno una
  deviazione standard);
\item
  \(K_e > K_c\) (acoplamento di Kuramoto interno superiore alla soglia
  critica --- esistono \emph{meccanismi formali} di coordinazione:
  partiti, federalismo, mercato, media);
\item
  \(\Phi_e \to 0\) malgrado (1) e (2) --- il parâmetro de ordem collassa
  per \emph{azione combinata} di rumore endogeno (polarizzazione
  affettiva), perdita di legitimidade (\(s_{iii} \downarrow\)) e sorgenti
  esogene \(\zeta(t)\);
\item
...
denominatore. La transição de fase è \textbf{estrutural}, non
congiunturale.
...
2026}{Misura Empirica della Decoerenza: \textbackslash Phi\^{}\{\textbackslash text\{USA\}\} tra 2024 e 2026}}\label{misura-empirica-della-decoerência-phitextusa-tra-2024-e-2026}
...
diretta di \(s_{iii}\) (legitimidade ético-discursiva) ed indiretta di
\(\Phi\) (la \emph{liberal democracy} esige sincronização interna fra
...
\(A_{\text{sys}} < 0\)}{Iper-estensione Esecutiva: A\_\{\textbackslash text\{sys\}\} \textless{} 0}}\label{hiperextensão-esecutiva-a_textsys-0}
...
L'antifragilidade è definita come
...
escalation per via alternativa} è la firma DEQ$^2$ dell'hiperextensão: la
...
Generosity & \(-68\) & crollo della \emph{qualidade ética} \(Q_0\)
macro \\
Ease of doing business & \(-21\) & crollo \(A\) percepito da terzi \\
Climate action support & \(-16\) & \(Q_0\) sull'asse \(z\) ---
complexidade dialética negata \\
Human rights & \(-10\) & crollo coordinata \(x\) --- coerência formal \\
...
equilibrio separante in cui si colloca il Soliton (Cap.~6.2). Lo Stato
egemone \emph{si solitonizza} discorsivamente, e perde assim la presa sui
Riceventi che ne avevano riconosciuto la legitimidade sotto il regime
precedente.
...
Il Cap.~6 ha modellato il Soliton (Musk) come sorgente esogena di
disordine \(\zeta_{\text{Sol}}(t)\) nel sistema USA. Il decorso fattuale
2025-2026 fornisce verificação empirica forte di questa modellazione.
...
\Hypoth{} \textbf{Tesi formale.} L'equação di Kuramoto modificata del
Cap.~6,
...
--- è ora verificaçãobile sul dataset 2025-2026 con strumenti statistici
...
egemone in decoerência, porque entrambi mancano di \(\Phi\). La
cooperazione produce somma, non interferenza costruttiva.



\section{Tariffe e Decoerenza
Monetaria}\label{tariffe-e-decoerência-monetaria}
...
sistema produttivo primeiro di colpire gli interlocutori. È un'azione che
presuppone \(A_{\text{sys}} > 0\) per essere razionale, enquanto la
diagnosi del \S{}7.2 dà \(A_{\text{sys}} < 0\). Si configura portanto come
\textbf{errore strategico estrutural}, non come scelta antifrágil.
...
Falsificazione}\label{midterm-2026-come-test-di-falsificação}
...
\Proved{} \textbf{In una frase:} os Estados Unidos del 2026 sono
l'archetipo estrutural dell'Hegemon Decoerente DEQ$^2$ --- un sistema con
capacidade material residua e meccanismi di acoplamento ancora
formalmente operanti, in cui \(\Phi\) è collassato per effetto
convergente di hiperextensão esecutiva (riduzione di
\(A_{\text{sys}}\)), polarização afetiva (rumore endogeno
irriducibile), crollo di \(s_{iii}\) (perdita di legitimidade esterna
...
\bottomrule\noalign{}
\endlastfoot
\(s_{ii}\) (capacidade material) & alta & stabile & mantenuta \\
\(s_{iii}\) (legitimidade) & \(74{,}9\) Brand Finance, \(0{,}57\) V-Dem &
\(\downarrow\downarrow\) & abbatte numeratore \\
\(\Phi\) (coerência de fase) & \(\to 0\) & crollo estrutural & annulla
numeratore \\
\(A_{\text{sys}}\) (antifragilidade) & \(< 0\) & concavo & amplifica
\(\sigma\) \\
...
agli indicatori 2026. La decoerência è asimmetrica com relação al colapso:
gli USA \emph{non} perdono la capacidade material (\(s_{ii}\) resta
alto), ma perdono la \textbf{funzione egemonica} --- la possibilità di
proiettare un parâmetro de ordem globale. Restano una \emph{potenza
materiale senza fase}, equivalente sistemico di una stella che continua
a brillare enquanto il suo nucleo si è già spento.
...
\bottomrule\noalign{}
\endlastfoot
1 & Definizione formale Hegemon Decoerente & \Hypoth{} categoria DEQ$^2$
nuova \\
