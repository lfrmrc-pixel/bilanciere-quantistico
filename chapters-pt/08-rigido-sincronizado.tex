\chapter{\texorpdfstring{O Rígido Sincronizado (China): Coerenza di
Comando $\neq$
\(\Phi\)}{O Rígido Sincronizado (China): Coerenza di Comando $\neq$ \textbackslash Phi}}\label{il-rigido-sincronizzato-cina-coerenza-di-comando-phi}
...
esterna anziché per sincronização spontanea), ma ciò \emph{non è}
l'ordine di Kuramoto. Il parâmetro de ordem vero è eroso da tre processi
convergenti --- Neijuan estrutural (\(\Omega \to \infty\)), colapso
demografico (riduzione catastrofica del sostrato di \(N\) futuro),
decoupling tecnologico forzato (auto-acoplamento incompleto) ---
enquanto la pressione su Taiwan misura il punto di rottura della rigidità.
\end{quote}



\section{Definição Estrutural del Rigido
Sincronizzato}\label{definição-estrutural-del-rigido-sincronizzato}
...
\item
  ...
  esterna} e non per acoplamento di Kuramoto autentico);
\item
  \textbf{\(\Phi_e^{\text{osservato}}\) alto,
  \(\Phi_e^{\text{spontaneo}}\) basso} --- esiste un divario estrutural
...
\item
  ...
  politico-discorsivo (zero opzionalità di dissenso, portanto zero
  apprendimento dagli errori sistemici);
\item
  \(s_{ii,e}\) alto, \(s_{iii,e}\) in calo \emph{all'esterno} (cresce
  internamente per saturação del campo informativo).
\end{enumerate}
...
senso di Thom}: la sua transição de fase sarà discontinua, non
graduale come quella dell'Hegemon Decoerente.
...
estrutural. Questa è una firma DEQ$^2$ inequivocabile di Rigido
...
Strutturale}{Neijuan (\textbackslash Omega) come Sostanza Strutturale}}\label{neijuan-omega-come-sostanza-estrutural}

\Hypoth{} \textbf{Tesi.} Il \emph{Neijuan} (, \emph{involução}) ---
...
variável DEQ$^2$ nel sistema chinês.
...
(al Q1 2026) & dissipazione produttiva estrutural \\
...
marginale a costo crescente, onde il rapporto \emph{energia dissipata /
energia utile} cresce monotonicamente.

\Conj{} \textbf{Sotto-tesi}. L'esistenza stessa di una \emph{campagna
ufficiale contro l'involução} è il momento DEQ$^2$ più rivelatore: in un
...
\section{Decoupling Tecnologico: Auto-acoplamento
Incompleto}\label{decoupling-tecnologico-auto-acoplamento-incompleto}
...
\tightlist
\item
  \textbf{Riduzione del campo \(\Pi\) globale} in cui il sistema chinês
  è inserito (perdita di varietà \(\Sigma\) --- eredità Topografia);
\item
  \textbf{Aumento di \(A^{\text{tecnologico}}\)} (resiliência alla
...
\Proved{} \textbf{Conseguenza per \(T_s^{(2),\text{sys}}\) chinês.}
L'auto-acoplamento aumenta numericamente \(\Phi^{\text{osservato}}\)
...
dal 1949}. Le nascite del 2025 sono il \(-17\%\) com relação al 2024,
...
\Hypoth{} \textbf{Lettura DEQ$^2$ formale.} Tutti i parâmetros del modello
...
\Proved{} \textbf{Conseguenza.} Il colapso demografico è \textbf{non
recuperabile} entro l'orizzonte 2031 della DEQ$^2$: i nati del 2026
entreranno nella forza lavoro após il 2046. La transição de fase
chinês è portanto \emph{parâmetroszzata da una variável lenta che ha già
attraversato la sua limiar crítico}. Il sistema si sta muovendo verso il
...
secondo cui un Rígido Sincronizado non può modificare una variável
...
qualidade ética, dimensioni \(x, y, z\) del progetto di vita). La leva del
...
Taiwan: media \(> 300/mese\) após maggio 2024 (più del doppio del
...
\(K^{\text{esterno}}\) chinês raggiunge il limite della sua
applicabilità. Tutte le altre variáveis (Hong Kong, mercato interno,
...
\Hypoth{} \textbf{Tesi formale.} L'escalation militare chinês su Taiwan
...
através de cui il Rígido Sincronizado può tentare di ricostituire
\(\Phi^{\text{osservato}}\) a basso costo materiale immediato --- al
prezzo di una transição de fase potenzialmente catastrofica del
sistema globale.
...
popolazione mondiale, \(35\%\) GDP globale. China-Rússia:
...
Presidenza BRICS 2026: Índia (CBDC interconnection in agenda, non
yuan-centrico).

\Hypoth{} \textbf{Lettura DEQ$^2$.} L'iniziativa chinês sui BRICS+ è un
...
l'acoplamento gerarchico a un sistema più ampio (multilaterale) per
acquisire legitimidade esterna sostitutiva di \(s_{iii}\) in calo.
...
produce \emph{correlazione}, non \emph{coerência de fase}. Tre indizi
convergenti:
...
Presidenza Índia 2026 spinge CBDC interconnection (non yuan) & Índia
...
\bottomrule\noalign{}
\endlastfoot
\(s_{ii}\) (capacidade material) & molto alta & stabile & mantenuta \\
\(s_{iii}\) (legitimidade esterna) & \(73{,}5\) Brand Finance, in salita
...
verso \(\Phi^{\text{spontaneo}}\). La China è portanto l'attore che,
nell'orizzonte 2028-2031 della DEQ$^2$, ha la \textbf{probabilità più alta
di una transição de fase non graduale}.

\Hypoth{} \textbf{Asimmetria con l'Hegemon Decoerente (Cap.~7).} USA e
China convergono entrambi a \(\Phi \to 0\) ma per percorsi opposti: gli
USA \emph{non hanno mai prodotto} la \(\Phi\) richiesta (decoerência
...
\tightlist
\item
  \textbf{Deflazione chinês si estende a \(> 12\) trimestri consecutivi
...
\item
  ...
  IMF trimestrali.
\item
  \textbf{Anti-involution campaign chinês non riduce \(\Omega\) entro
...
\bottomrule\noalign{}
\endlastfoot
1 & Definizione formale Rígido Sincronizado (5 condizioni) & \Hypoth{}
...
5 & Stack DeepSeek-Huawei come auto-acoplamento forzato & \Proved{}
dato aprile 2026 \\
6 & Demografia come variável lenta oltre limiar crítico & \Proved{}
dato 2025-2026 (RAND, Worldometer) \\
7 & Taiwan come valvola di compensazione \(\Phi^{\text{spontaneo}}\) &
\Conj{} hipótese estrutural forte \\
...
11 & Asimmetria USA/China (decoerência distribuita vs rigidità
catastrofica) & \Hypoth{} risultato estrutural comparativo \\
\end{longtable}}
}
...
\item
  ...
  2025
\item
  Xiang Biao (Oxford / Max Planck) --- definição di Neijuan come
  \emph{dead loop}
\item
...
\item
  BRICS Pay slittato a 2030; presidenza Índia 2026 con CBDC focus
\end{itemize}
