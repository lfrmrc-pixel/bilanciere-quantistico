\chapter{Predições Arriscadas Datadas: Falsificabilità
...
falsificação \emph{datata} è indistinguibile da una narrazione. Questo
capítulo aggrega le sedici predições arriscadas della DEQ$^2$ --- tredici
derivate dai Cap.~7-9 sui tre attori sistemici (cinque USA, quattro
China, quattro Brasil), tre derivate dal Cap.~10 sull'aggregato globale
...
ciascuna predizione è além disso specificato \emph{cosa accadrebbe alla
teoria se fallisse}: la falsificação di alcune predizioni è gestibile
come correção local, quella di altre forzerebbe la riformulazione
estrutural dell'apparato.
\end{quote}



\section{Critérios DEQ$^2$ per una Predizione
...
\item
  ...
  verificaçãobile da terzi indipendenti;
\item
...
\item
  ...
  \(\mathcal{P}\) fallisce --- \emph{correção local},
...
aggiornamento esplicito delle probabilità posteriori sui parâmetros della
...
\subsection{USA --- Hegemon Decoerente (Cap.
...
\(0{,}70\) & local (parâmetro \(\Phi\)) \\
\textbf{U2} & Quota dollaro nelle riserve IMF COFER \(< 55\%\) & Q4 2027
& \(0{,}55\) & IMF COFER trimestrale & \(0{,}55\) & local (parâmetro
\(\sigma\) esogena) \\
...
\(0{,}55\) & parâmetrosca (\(\Omega\) acuta vs cronica) \\
...
\bottomrule\noalign{}
\endlastfoot
\textbf{C1} & Deflazione chinês \(> 12\) trimestri consecutivi & Q4 2026
...
\(0{,}50\) entro intervallo & estrutural se \(P\) fuori intervallo \\
...
\textbf{C4} & PPI chinês in territorio negativo \(\geq 5\) anni cumulati
& Q4 2027 & \(5\) anni & NBS PPI mensile & \(0{,}70\) & local
(anti-involution efficacia) \\
\end{longtable}}
}

\subsection{Brasil --- Balança Quântica (Cap.
...
ufficiali & \(0{,}60\) condicional & estrutural (la
...
predizione local aggiusterebbe parâmetros specifici; il fallimento di
una predizione estrutural costringerebbe a ripensare l'apparato
corrispondente.
...
2031 & \(0{,}80\) & estrutural (forma di \(T_s^{(2),\text{sys}}\)) \\
\textbf{G2} &
...
observação 2029-2031 &
\(\Delta(d^{\text{oss}} - d^{\text{spont}}) > 0{,}10\) in 24 mesi &
dashboard SPC-DEQ + V-Dem + NBS & \(0{,}55\) & estrutural (definição
di \emph{Rígido Sincronizado} del Cap.~8.0) \\
\end{longtable}}
}

\Hypoth{} \textbf{Lettura.} G1 e G2 sono \emph{complementari}: la teoria
afferma simultaneamente che il sistema globale 2031 sarà in decoerência
...
conferma di G1 con falsificação di G2 (\(\Pr \approx 0{,}25\))
...
2026}\label{la-predizione-meta-lelezione-brasileira-del-4-ottobre-2026}
...
atualização bayesiana della probabilità del Mondo M1/M2 \\
...
$\to$ B1 vera & falsificação binaria \\
Ottobre/novembre 2026 (eventuale 2$^\circ$ turno) & Risultato secondo turno:
vince A/B o C? & falsificação binaria definitiva \\
...
Falsificazione}\label{protocollo-di-pubblicazione-e-falsificação}
...
(target 2026 Q3 --- KDP IT/EN/ES/PT). La pré-registro consiste
...
\tightlist
\item
  \textbf{Tabela delle predizioni} (sezioni 11.1, 11.2) con tutti i
  campi (1)-(5);
\item
  ...
  \item
    log dei \emph{checkdepoisnt} trimestrali con re-stima probabilità
    soggettive;
  \end{itemize}
\item
  ...
  \item
    atualização bayesiana delle probabilità;
  \item
    eventuali correzioni alla teoria (locali/parâmetrosche/strutturali).
  \end{itemize}
\end{enumerate}
...
\emph{tutti} gli assi --- incluso \(z\) (complexidade dialética) por meio de
l'esposizione esplicita alla falsificação.
...
del Cap.~6.6 va riformulato. Locale-parâmetrosca. \\
C1 & Deflazione finisce primeiro: anti-involution funziona. Aggiustamento
\(\Omega^{\text{Cina}}\). Locale. \\
C2 & Nessun bloqueo malgrado condizioni soddisfatte: hipótese
\emph{Taiwan come valvola} va rivista. \textbf{Strutturale}. \\
C3 & Yuan oltre \(5\%\) entro 2030: la dedollarizzazione converge sullo
yuan, contro la tesi Cap.~8.6. Locale-parâmetrosca. \\
C4 & PPI positivo primeiro del 2027: anti-involution efficace.
...
definição di \(\Phi\)-engine è insufficiente. \textbf{Strutturale}. \\
...
U5). Il fallimento di una predizione estrutural impone
...
invalidare l'apparato; il libro è portanto un \emph{pre-registered
prediction set} il cui valore epistemico cresce o diminuisce in
proporzione alla performance verificaçãobile della teoria.
...
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Tipo di falsificação
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Cap.~7 USA & 5 & \(0{,}60\) & 4 locali, 1 parâmetrosca/meta (U5) \\
Cap.~8 China & 4 & \(0{,}60\) & 3 locali, 1 estrutural (C2) \\
Cap.~9 Brasil & 4 & meta + condizionali & 2 condizionali, 1 estrutural
(B4), 1 meta (B1) \\
...
libro (Cap.~12-13) costruirà em vez disso il \emph{contenuto normativo}: dato
...
\bottomrule\noalign{}
\endlastfoot
1 & Cinque criteri DEQ$^2$ per predição arriscada & \Hypoth{}
metodologia \\
...
architettura logica \\
5 & Protocollo di pré-registro e repository GitHub & \Hypoth{}
...
\section{Chiusura della Parte IV e Abertura da Parte
...
Cap.~11 ha aggregato le 16 predições arriscadas datate con criteri
popperiani operativi e protocollo di falsificação pubblica.
...
\item
  ...
  50+ paesi), inviti a falsificação.
\end{itemize}
