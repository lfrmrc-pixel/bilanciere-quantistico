...
mensurável in uno spazio tridimensionale (coerência formal, precisione
tecnica, complexidade dialética); la sua efficacia sociale è il prodotto
di una \emph{massa comunicativa} per una \emph{qualidade ética} diviso una
\emph{resistência crítica}; il sistema discorsivo è governato da una
lei preditiva dalla quale, per transposição de escala, deriverà
l'intera DEQ$^2$.
\end{quote}
...
può sembrare un'inversione di scala. Non lo è: l'egemonia \emph{è} primeiro
di tutto una struttura comunicativa, e la geopolitica del XXI secolo è
una geopolitica di flussi semantici primeiro ancora che di flussi
...
equação applicata a due scale}. Per leggere quella demonstração, il
...
Il referência esteso è: Marcelino, \emph{La Topografia del Discorso
...
La Escola de Frankfurt ha prodotto un apparato critico di
straordinaria potenza --- Adorno, Horkheimer, Marcuse, Habermas --- ma
\textbf{senza strumenti operativi di medição}. Si poteva
\emph{descrivere} l'unidimensionalità del pensiero (Marcuse),
l'indústria cultural (Adorno), la regressione dell'ascolto
(Horkheimer), la colonização do mundo da vida (Habermas) --- ma
non \emph{misurarli}.
...
\section{\texorpdfstring{Lo Espaço Discursivo
\(\mathcal{D} = [0, 10]^3\)}{Lo Espaço Discursivo \textbackslash mathcal\{D\} = {[}0, 10{]}\^{}3}}\label{lo-spazio-discorsivo-mathcald-0-103}
...
\tightlist
\item
  \(x\) = \textbf{coerência formal} (consistenza interna, assenza di
  contraddizioni logiche, struttura argomentativa);
\item
  \(y\) = \textbf{precisão técnica} (rigore fattuale, accuratezza dei
  riferimenti, padronanza disciplinare);
\item
  \(z\) = \textbf{complexidade dialética} (capacità di pensare le
...
(é possível essere coerenti senza essere profondi --- il caso della
propaganda razionale).

\subsection{\texorpdfstring{Il Vetor Ético
\(\mathbf{E}\)}{Il Vetor Ético \textbackslash mathbf\{E\}}}\label{il-vettore-etico-mathbfe}
...
neutra --- incorpora la scelta di campo della Escola de Frankfurt:
...
onde: \(K_{\text{cap}}\) = capitale economico, \(R\) = reach (in
miliardi), \(A_{\text{alg}}\) = amplificação algorítmica, \(S\) =
saturação (ore/giorno), \(T\) = durata (mesi). L'esponente \(0{,}8\)
su \(R\) riflette i retornos decrescentes del reach puro.

\Hypoth{} \textbf{Disambiguazione notazionale.} Il simbolo
\(K_{\text{cap}}\) è usato per il capitale economico per distinguerlo
dal \(K\) del Cap.~4 --- \emph{forza di acoplamento di Kuramoto},
parâmetro completamente diverso. Analogamente, \(A_{\text{alg}}\) resta
riservato all'amplificação algorítmica, enquanto \(A\) (senza pedice)
sarà introdotto nel Cap.~3 come \emph{antifragilidade} (Taleb).
...
onde \(z_r \in [0, 10]\) è la complexidade dialética media del pubblico,
...
la minimizzazione dei prerequisiti cognitivi, portanto la compressione di
...
dialetticamente ricco ha qualidade ética superiore. La complessità
...
Per ogni massa comunicativa, esiste una resistência crítica sufficiente a
...
L'ótimo de Pareto è (Dialogo, Critico) \(= (5, 5)\) --- l'agire
comunicativo habermasiano. (Propaganda, Passivo) \(= (10, -5)\) è la
\textbf{Barbárie Tecnológica}.
...
equilíbrio de Nash perfeito em subjogos se e solo se:
...
egemoni. Questa identità non è coincidenza --- è l'invariância de escala
...
Il termine \((1 - \delta_R z_R)\) è il \textbf{fator de erosão}:
misura quanto la resistência crítica del ricevente riduce il payoff della
...
\item
  \(\delta_R z_R > 1\): \textbf{efeito bumerangue} --- la pressione
  comunicativa rinforza la resistenza (\(\Pi < 0\)).
\end{itemize}

\Proved{} Questa equação è la \textbf{lei micro fundamental}
...
Theorem della Geometria --- invariância de escala & Cap.~3 \S{}3.1.3 \\
Equilibrio separante bayesiano \((8, 9, 2)\) & Microestrutura della
strategia del Soliton (Musk) & Cap.~6 \\
$\Pi$(t+1) micro & Legge local del campo do qual \(T_s^{(2),\text{sys}}\)
emerge & Cap.~3-4 \\
\end{longtable}}
}
...
fenomenologica di Clausius: la microfundação che spiega da dove
...
\bottomrule\noalign{}
\endlastfoot
1 & Spazio \(\mathcal{D}\) e vetor ético \(\mathbf{E}\) & \Proved{}
definição operativa & Topografia \S{}4 \\
...
\Proved{} verificaçãodo (Cap.~3) & DEQ$^2$ Cap.~3 \S{}3.1.3 \\
7 & $\Pi$(t+1) come lei micro fundamental & \Hypoth{} hipótese estrutural
