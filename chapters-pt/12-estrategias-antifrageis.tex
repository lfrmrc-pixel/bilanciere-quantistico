...
\section{O Fundamento Evolutivo das Estrat\'{e}gias: Ostrom e os Commons de \(\Phi\)}\label{il-fondamento-evolutivo-delle-strategie}

\Proved{} \textbf{A base emp\'{i}rica (n\~{a}o normativa) da
coopera\c{c}\~{a}o.} As estrat\'{e}gias do Cap.~12 prescrevem a\c{c}\~{o}es
que aumentam $\Phi$ e reduzem $\Omega$. Mas por que essas a\c{c}\~{o}es
s\~{a}o \emph{poss\'{i}veis} --- e n\~{a}o apenas desej\'{a}veis? A
resposta \'{e} oferecida por Elinor Ostrom (1990): os \emph{commons} ---
recursos coletivos --- \emph{podem} ser geridos cooperativamente a longo
prazo quando s\~{a}o satisfeitas condi\c{c}\~{o}es institucionais
precisas. A coopera\c{c}\~{a}o n\~{a}o \'{e} inevit\'{a}vel, nem
ut\'{o}pica: \'{e} \emph{condicionalmente est\'{a}vel} sob
institui\c{c}\~{o}es bem projetadas.

Este \'{e} o fundamento normativo correto: n\~{a}o Kropotkin (``a
coopera\c{c}\~{a}o est\'{a} na natureza'', que incorre na fal\'{a}cia
naturalista) mas Ostrom (``a coopera\c{c}\~{a}o \'{e} empiricamente
documentada sob estas condi\c{c}\~{o}es'').

\Hypoth{} \textbf{Os oito princ\'{i}pios de Ostrom como condi\c{c}\~{o}es
para \(\Phi > 0\).} Ostrom (1990) identifica oito princ\'{i}pios de
design para a gest\~{a}o sustent\'{a}vel dos commons. No DEQ$^2$, esses
princ\'{i}pios se traduzem diretamente em condi\c{c}\~{o}es para
$\Phi > 0$ e $\phi < \phi^*$:
...
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Princ\'{i}pio Ostrom (1990)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Correspond\^{e}ncia CPGG/DEQ$^2$ (\Hypoth{})
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
1. Fronteiras claras & Reduz $\phi$ impedindo infiltra\c{c}\~{a}o extrativa \\
2. Regras adaptadas ao contexto & Estabiliza $\sigma_{\text{san}}$ localmente \\
3. Participa\c{c}\~{a}o na elabora\c{c}\~{a}o das regras & Aumenta $\kappa_C$ (coes\~{a}o dos cooperadores) \\
4. Monitoramento & Reduz $\kappa_E$ (centraliza\c{c}\~{a}o dos extratores) \\
5. San\c{c}\~{o}es graduadas & Mant\'{e}m $\sigma_{\text{san}} > \sigma_{\text{crit}}$ \\
6. Mecanismos de resolu\c{c}\~{a}o de conflitos & Reduz $\varepsilon_{\text{dis}}$ \\
7. Reconhecimento externo & Aumenta $s_{iii}$ (legitimidade \'{e}tico-discursiva) \\
8. Governan\c{c}a aninhada & Produz $\Phi$ em m\'{u}ltiplos n\'{i}veis (escalabilidade) \\
\end{longtable}}
}

\Proved{} \textbf{Interpreta\c{c}\~{a}o.} Os vinte e sete protocolos
do Cap.~12.2--12.4 s\~{a}o implementa\c{c}\~{o}es contextuais dos oito
princ\'{i}pios de Ostrom na realidade geopol\'{i}tica 2026--2031. N\~{a}o
s\~{a}o prescri\c{c}\~{o}es ideais: s\~{a}o protocolos operativos para
manter $\phi < \phi^*$ enquanto o sistema global navega em dire\c{c}\~{a}o
\`{a} bifurca\c{c}\~{a}o 2028--2031.

\Hypoth{} \textbf{O limite: o problema da escala.} Os commons de Ostrom
s\~{a}o locais (lagos, florestas, sistemas de irriga\c{c}\~{a}o). A
extens\~{a}o \`{a} escala geopol\'{i}tica requer a hip\'{o}tese de que
os princ\'{i}pios de design escalonem --- hip\'{o}tese apoiada por
evid\^{e}ncia parcial (BRICS+ como tentativa de commons
energ\'{e}tico/financeiro em escala regional) mas ainda n\~{a}o demonstrada
em escala global. Esta \'{e} a verdadeira aposta do per\'{i}odo
2026--2031: a primeira prova emp\'{i}rica da escalabilidade dos commons
de Ostrom \`{a} escala hegem\^{o}nica.
