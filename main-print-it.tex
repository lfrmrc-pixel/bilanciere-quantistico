...
\textbf{Nota sul fondamento evolutivo della cooperazione.}
La DEQ$^2$ descrive \textit{dove} i sistemi egemonici vanno --- ma richiedeva, fino alla presente edizione, una risposta al \textit{perché}. Il perché è offerto dalla biologia evolutiva. La condizione di Hamilton (1964) --- $rB > C$ --- stabilisce quando la cooperazione è evolutivamente stabile in una popolazione. L'equazione di Price (1970) decompone algebricamente la variazione del comportamento cooperativo in un termine inter-gruppo (proporzionale alla coerenza $\Phi$) e un termine intra-gruppo (proporzionale all'involuzione $\Omega$). La teoria della selezione multilivello (Wilson \& Sober 1994; Okasha 2006) giustifica formalmente il cambio di livello descrittivo che la DEQ$^2$ opera. Nowak (2006) fornisce la tassonomia diagnostica dei meccanismi cooperativi; Ostrom (1990) il fondamento empirico per la parte normativa.

Il \textit{Captured Public Goods Game} (CPGG, Appendice D §D.4) formalizza il meccanismo per cui una sotto-rete estrattiva di frazione $\phi$ degrada simultaneamente tutti i meccanismi cooperativi, abbassando il grado efficace $k_\text{eff}(\phi)$ della rete cooperatrice e portando $\Phi \to 0$. Da questa analisi si deriva --- non si postula --- la formula corretta $\Omega \approx \phi\eta r/(r-1)$ (Cap. 3). Il monitoraggio di $\phi$ diventa così il primo indicatore precoce di $T_s^{(2),\text{sys}} \to 0$.

Kropotkin (1902) aveva anticipato fenomenologicamente la tesi centrale: la cooperazione è lo stato naturale, la decoerenza è lo stato prodotto. Il formalismo del CPGG offre oggi la derivazione rigorosa di quella intuizione.
...
\input{chapters-it/01-topografia}
\input{chapters-it/02-geometria}
\input{chapters-it/03-equazione-unificata}
...
\input{chapters-it/04-forma-sistemica}
\input{chapters-it/05-quattro-variabili}
...
\input{chapters-it/10-dashboard-spc}
\input{chapters-it/11-predizioni-datate}
...
\input{chapters-it/13-conclusioni}
...
\input{chapters-it/AppA-derivazione-formale}
\input{chapters-it/AppB-calibrazione-sigma}
\input{chapters-it/AppC-protocolli-misurazione}
\input{chapters-it/AppD-dialogo-tradizioni}
