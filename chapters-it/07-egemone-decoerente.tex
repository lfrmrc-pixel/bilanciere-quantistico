\chapter{L'Egemone Decoerente (USA): Iper-estensione senza
Coerenza}\label{legemone-decoerente-usa-iper-estensione-senza-coerenza}

\begin{quote}
\textbf{Argomento portante:} gli Stati Uniti del secondo mandato Trump
esibiscono il pattern formale dell'\emph{egemone decoerente}: massima
capacità materiale \(s_{ii}\) residua, accoppiamento sistemico interno
\(K > K_c\), ma parametro d'ordine \(\Phi \to 0\) per effetto combinato
di iper-estensione esecutiva, polarizzazione affettiva, declino di
\(s_{iii}\) e azione di sorgenti esogene di disordine \(\zeta(t)\) ---
fra cui il Solitone Musk del Cap. 6. Il sistema è formalmente in regime
sub-critico nonostante il superamento della soglia di accoppiamento.
\end{quote}



\section{Definizione Strutturale dell'Egemone
Decoerente}\label{definizione-strutturale-dellegemone-decoerente}

\Hypoth{} \textbf{Definizione (Egemone Decoerente DEQ$^2$).} Un attore
\(e\) è detto \emph{Egemone Decoerente} se soddisfa simultaneamente:
...
\item
  ...
  (capacità materiale residua sopra la media sistemica per almeno una
  deviazione standard);
\item
  \(K_e > K_c\) (accoppiamento di Kuramoto interno superiore alla soglia
  critica --- esistono \emph{meccanismi formali} di coordinazione:
  partiti, federalismo, mercato, media);
\item
  \(\Phi_e \to 0\) malgrado (1) e (2) --- il parametro d'ordine collassa
  per \emph{azione combinata} di rumore endogeno (polarizzazione
  affettiva), perdita di legittimità (\(s_{iii} \downarrow\)) e sorgenti
  esogene \(\zeta(t)\);
\item
...
\Proved{} \textbf{Conseguenza.} \(T_s^{(2),\text{sys}} \to 0\) per
\(\Phi \to 0\) al numeratore e \(\Omega/\Phi \to \infty\) al
denominatore. La transizione di fase è \textbf{strutturale}, non
congiunturale.
...
2026}{Misura Empirica della Decoerenza: \textbackslash Phi\^{}\{\textbackslash text\{USA\}\} tra 2024 e 2026}}\label{misura-empirica-della-decoerenza-phitextusa-tra-2024-e-2026}

\Proved{} \textbf{Indicatori convergenti del collasso di \(\Phi\).}
...
\Hypoth{} \textbf{Interpretazione formale.} L'indice V-Dem è proxy
diretta di \(s_{iii}\) (legittimità etico-discorsiva) ed indiretta di
\(\Phi\) (la \emph{liberal democracy} esige sincronizzazione interna fra
esecutivo, legislativo, giudiziario, civil society). Il collasso
...
\(A_{\text{sys}} < 0\)}{Iper-estensione Esecutiva: A\_\{\textbackslash text\{sys\}\} \textless{} 0}}\label{iper-estensione-esecutiva-a_textsys-0}
...
\Proved{} \textbf{Conseguenza formale per \(A_{\text{sys}}\).}
L'antifragilità è definita come
...
review, civil service indipendente). La risposta diventa concava: ogni
...
escalation per via alternativa} è la firma DEQ$^2$ dell'iper-estensione: la
...
Stati Uniti registrano \textbf{il calo più ripido tra tutti i 193 Stati
misurati}: \(-4{,}6\) punti su scala 100, da \(\approx 79{,}5\) a
\(74{,}9\). La Cina sale a \(73{,}5\), riducendo a meno di \(1{,}5\)
...
Generosity & \(-68\) & crollo della \emph{qualità etica} \(Q_0\)
macro \\
Ease of doing business & \(-21\) & crollo \(A\) percepito da terzi \\
Climate action support & \(-16\) & \(Q_0\) sull'asse \(z\) ---
complessità dialettica negata \\
Human rights & \(-10\) & crollo coordinata \(x\) --- coerenza formale \\
Ethical standards & \(-4\) & erosione del baseline \(Q_0\) \\
\end{longtable}}
}

\Hypoth{} \textbf{Modellazione formale.} \(s_{iii}^{\text{USA}}\) è la
...
equilibrio separante in cui si colloca il Solitone (Cap. 6.2). Lo Stato
egemone \emph{si solitonizza} discorsivamente, e perde così la presa sui
Riceventi che ne avevano riconosciuto la legittimità sotto il regime
precedente.
...
dialettica, riconoscimento dell'altro). Il loro collasso simultaneo a
...
Il Cap. 6 ha modellato il Solitone (Musk) come sorgente esogena di
disordine \(\zeta_{\text{Sol}}(t)\) nel sistema USA. Il decorso fattuale
2025-2026 fornisce verifica empirica forte di questa modellazione.
...
decoerenza economica \\
Ban su Nvidia H20 chip in Cina, \(5{,}5\)B charge & Q1 2025 & frizione
...
\Hypoth{} \textbf{Tesi formale.} L'equazione di Kuramoto modificata del
Cap. 6,
...
formale (il sistema partitico, il sistema federale, il sistema mediatico
esistono ancora). La predizione del Cap. 6.6 --- \emph{correlazione
\(r > 0{,}3\) tra Musk visibility e polarizzazione USA nel 2022-2026}
--- è ora verificabile sul dataset 2025-2026 con strumenti statistici
...
realizzazione macro della \textbf{non-trasferibilità del Solitone} (Cap.
6.5): un Solitone non può co-coordinarsi stabilmente con un sistema
egemone in decoerenza, perché entrambi mancano di \(\Phi\). La
cooperazione produce somma, non interferenza costruttiva.



\section{Tariffe e Decoerenza
Monetaria}\label{tariffe-e-decoerenza-monetaria}
...
sistema produttivo prima di colpire gli interlocutori. È un'azione che
presuppone \(A_{\text{sys}} > 0\) per essere razionale, mentre la
diagnosi del \S{}7.2 dà \(A_{\text{sys}} < 0\). Si configura dunque come
\textbf{errore strategico strutturale}, non come scelta antifragile.
...
Falsificazione}\label{midterm-2026-come-test-di-falsificazione}
...
nei primi 12 mesi del Congresso 119$^\circ$ (a partire da gennaio 2027):
...
\Proved{} \textbf{In una frase:} gli Stati Uniti del 2026 sono
l'archetipo strutturale dell'Egemone Decoerente DEQ$^2$ --- un sistema con
capacità materiale residua e meccanismi di accoppiamento ancora
formalmente operanti, in cui \(\Phi\) è collassato per effetto
convergente di iper-estensione esecutiva (riduzione di
\(A_{\text{sys}}\)), polarizzazione affettiva (rumore endogeno
irriducibile), crollo di \(s_{iii}\) (perdita di legittimità esterna
documentata da Brand Finance e V-Dem) e azione persistente del Solitone
...
\bottomrule\noalign{}
\endlastfoot
\(s_{ii}\) (capacità materiale) & alta & stabile & mantenuta \\
\(s_{iii}\) (legittimità) & \(74{,}9\) Brand Finance, \(0{,}57\) V-Dem &
\(\downarrow\downarrow\) & abbatte numeratore \\
\(\Phi\) (coerenza di fase) & \(\to 0\) & crollo strutturale & annulla
numeratore \\
\(A_{\text{sys}}\) (antifragilità) & \(< 0\) & concavo & amplifica
\(\sigma\) \\
...
& \(\uparrow\) & forza decoerenza \\
\end{longtable}}
}

\Hypoth{} \textbf{Tesi conclusiva del capitolo.}
\(T_s^{(2),\text{sys}\,,\text{USA}} \to 0\) entro la finestra
\textbf{2027-2029}, con probabilità superiore a \(0{,}65\) condizionale
agli indicatori 2026. La decoerenza è asimmetrica rispetto al collasso:
gli USA \emph{non} perdono la capacità materiale (\(s_{ii}\) resta
alto), ma perdono la \textbf{funzione egemonica} --- la possibilità di
proiettare un parametro d'ordine globale. Restano una \emph{potenza
materiale senza fase}, equivalente sistemico di una stella che continua
a brillare mentre il suo nucleo si è già spento.
...
\bottomrule\noalign{}
\endlastfoot
1 & Definizione formale Egemone Decoerente & \Hypoth{} categoria DEQ$^2$
nuova \\
