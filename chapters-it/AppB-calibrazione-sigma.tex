\chapter{\texorpdfstring{Calibrazione Empirica di
\(\Sigma^{\text{geo}}\) a Partire da
\(\Sigma^{\text{comm}}\)}{Calibrazione Empirica di \textbackslash Sigma\^{}\{\textbackslash text\{geo\}\} a Partire da \textbackslash Sigma\^{}\{\textbackslash text\{comm\}\}}}\label{calibrazione-empirica-di-sigmatextgeo-a-partire-da-sigmatextcomm}

\begin{quote}
\textbf{Argomento portante:} la matrice di covarianza degli stati
egemonici \((s_i, s_{ii}, s_{iii})\) può essere stimata, in prima
...
sistema egemonico --- necessaria, ad esempio, per definire \(\sigma\)
...
pubblicazione precedente. La presente appendice propone una procedura di
...
prima approssimazione --- non lineare in regimi estremi (collasso,
transizione di fase) ma adeguata in regimi quasi-stazionari, che è il
...
identificando \(J\), otteniamo \(\Sigma^{\text{geo}}\) senza necessità
...
Identitaria}\label{strategia-a-ipotesi-nulla-identitaria}
...
non-ortogonalità si conserva sotto aggregazione. È l'ipotesi più
...
\tightlist
\item
  \(\alpha_1\): peso della coerenza formale micro \(x\) nella coesione
  interna macro \(s_i\);
\item
  \(\alpha_2\): peso della precisione tecnica micro \(y\) nella capacità
  materiale macro \(s_{ii}\);
\item
  \(\alpha_3\): peso della complessità dialettica micro \(z\) nella
  legittimità macro \(s_{iii}\).
\end{itemize}
...
Ipotesi più generale: \(J\) è \(3 \times 3\) piena, \(9\) parametri
liberi. Permette di catturare effetti come ``il discorso tecnico genera
capacità materiale ma anche coesione'' (\(J_{12} > 0\)) o ``la
complessità dialettica produce legittimità ma erode coesione''
(\(J_{31} > 0,\ J_{11}\) ridotto).

\Conj{} \textbf{Limite.} \(9\) parametri richiedono almeno 9 punti dati
...
dove la base dati può essere costruita.
...
\bottomrule\noalign{}
\endlastfoot
\(s_i\) --- coesione interna & World Bank Government Effectiveness &
Polity IV, V-Dem Liberal Democracy & WB / Polity / V-Dem \\
\(s_{ii}\) --- capacità materiale & PIL nominale $\times$ Economic Complexity
Index & Spesa militare, brevetti, energia & WB / IMF / OEC \\
\(s_{iii}\) --- legittimità etico-discorsiva & Brand Finance Soft Power
...
standardizzate per coerenza formale, precisione tecnica, complessità
dialettica.
...
materiale) è il più alto: la precisione tecnica del discorso si traduce
\emph{più che proporzionalmente} in capacità materiale macroscopica.
...
Questo è il livello base di volatilità aggregata da cui le predizioni
del Cap. 11 partono.

\subsection{\texorpdfstring{Calibrazione di
\(\Phi\)}{Calibrazione di \textbackslash Phi}}\label{calibrazione-di-phi}

\Hypoth{} La densità di accoppiamento \(K\) del Cap. 4 può essere
...
(cooperazione interna effettiva), e dunque sono più probabilmente sopra
...
dovrebbe avere \(R^2 \geq 0{,}75\) rispetto alla \(\Sigma^{\text{geo}}\)
...
\Hypoth{} \textbf{Questa appendice è il nucleo del paper tecnico EN.}
...
datate & alta \\
\emph{European Physical Journal B} & Modelli di sincronizzazione
(Kuramoto) & alta \\
\end{longtable}}
}
...
Appendice}\label{note-epistemiche-di-chiusura-appendice}
...
ipotesi di linearità, regime quasi-stazionario \\
...
4 & Strategia B (diagonale scalata) & \Hypoth{} ipotesi operativa con
...
verificare empiricamente nel paper EN \\
