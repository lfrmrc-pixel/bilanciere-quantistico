...
\section{Interpretazione Evolutiva delle Quattro
Variabili}\label{interpretazione-evolutiva-quattro-variabili}

\Hypoth{} \textbf{Le quattro variabili come firma evolutiva.} Ogni
variabile di \(\mathbb{V}^{(4)}\) ha una definizione fenomenologica
(Cap. 5) e una \emph{microfondazione causale evolutiva} (§D.4, framework
CPGG). Le due letture non si sostituiscono: si stratificano, offrendo
all'analista strumenti di diagnosi a diversi livelli di astrazione.
...
Variabile
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Definizione DEQ$^2$ (Cap. 3-5)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Microfondazione evolutiva (\Hypoth{})
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\(\Phi\) & Parametro d'ordine di Kuramoto: allineamento di fase del campo
\(\Pi\) & Firma sistemica del regime \(rB > C\) (Hamilton 1964):
\(\Phi > 0\) quando i meccanismi cooperativi prevalgono
sull'estrazione \\ \addlinespace
\(A\) & Convessità di \(\Pi\) rispetto alla volatilità \(\sigma\) & Robustezza
della condizione \(rB > C\) alle perturbazioni: \(A > 0\) indica che il
sistema guadagna cooperazione dagli shock invece di perderla \\ \addlinespace
\(\Omega\) & Rapporto energia dissipata/utile (Neijuan) & Coefficiente
CPGG derivato: \(\Omega \approx \phi\eta r/(r-1)\); misura la quantità di
estrazione che la sotto-rete \(\mathcal{E}\) opera sul campo cooperativo
\\ \addlinespace
\(\varepsilon_{\text{dis}}\) & Coefficiente di disimpegno morale
(Bandura/Milgram) & Degradazione endogena delle istituzioni che mantengono
\(rB > C\): aumenta con \(\phi > \phi^*\) attraverso la sigmoide
\(\mathcal{S}(\mu[\phi - \phi^*])\) \\
\end{longtable}}
}

\Proved{} \textbf{Lettura integrata.} Un sistema con \(\Phi\) alto, \(A\)
positivo, \(\Omega\) basso e \(\varepsilon_{\text{dis}}\) contenuto è un
sistema in cui: (i) la condizione \(rB > C\) vale per la maggioranza dei
sotto-attori; (ii) il sistema è robusto agli shock cooperativi; (iii) la
quota estrattiva \(\phi\) è sotto la soglia critica \(\phi^*\); (iv) le
istituzioni normative non sono degradate. È esattamente la posizione del
\textbf{Bilanciere Quantistico} nel Cap. 9 --- e ora sappiamo
\emph{perché} quella posizione è stabile, non solo \emph{che} è stabile.

\Hypoth{} \textbf{Il sistema come commons di Ostrom a scala geopolitica
(\Hypoth{}).} Il problema normativo del Cap. 12 si traduce, nella lettura
evolutiva, nel progettare le condizioni istituzionali --- gli otto
principi di Ostrom (1990) scalati alla geopolitica --- che mantengono
\(\phi < \phi^*\) e dunque \(\Phi > 0\) nel lungo periodo. La DEQ$^2$
offre la diagnostica; l'apparato evolutivo offre il \emph{perché} quella
diagnostica sia necessaria.
